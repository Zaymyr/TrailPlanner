...
export function AidStationBadge({ step, variant, className, ...props }: AidStationBadgeProps) {
  ...
  return (
    ...
    >
      ...
      />
      <span className="absolute bottom-1 left-1/2 -translate-x-1/2 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full border border-border bg-card px-1 text-[11px] font-semibold text-foreground shadow dark:border-slate-700 dark:bg-slate-950/90 dark:text-white">
        {step}
      </span>
    </div>
  );
