...
- Primary section selections from the contextual RaceBook bottom bar keep emitting `racebook tab viewed`. Its explicit Courses exit emits `racebook action clicked` with the bounded action `exit_to_catalog`; it carries no organizer-authored content or sponsor identity.
