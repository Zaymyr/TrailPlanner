...
- Merging every organizer-managed format into the mobile catalog, including masked and private presentation states, is a visibility/read change rather than a new analytics event. Existing course and RaceBook events keep stable event/race ids and must not record membership ids or private visibility state.
