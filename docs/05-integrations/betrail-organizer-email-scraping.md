...
related_files:
  ...
  - scripts/catalog-research-claim-ledger.mjs
  - scripts/catalog-research-claim-ledger.test.mjs
  - scripts/catalog-research-document-blocks.mjs
  - scripts/catalog-research-document-blocks.test.mjs
  - scripts/catalog-research-crawl-identity.test.mjs
  - scripts/catalog-research-event-merge.test.mjs
  - scripts/catalog-research-format-parser.test.mjs
  - scripts/catalog-research-queue-input.test.mjs
  - scripts/catalog-research-benchmark.mjs
...
The prospect CSV is a discovery source, not verified catalog data. `build-format-import-queue.mjs` creates one row per format, parses ISO/French dates and spreadsheet serial planning dates, and preserves the original candidates. Compact lists such as `12 / 23 / 51 km` become separate research candidates. Miles retain their original value/unit beside an explicitly candidate-only kilometre conversion. Timed, loop, relay, duo/team, VK, marathon and half-marathon labels are retained as format metadata without inventing a publishable distance or participation mode. Existing simple entries keep their stable format keys. `target_edition_year` comes from the campaign/prospect date, never from an old BeTrail URL. Invalid calendar dates and past exact editions are excluded. Planning dates and parsed format hints only order research; they cannot certify catalog values.
...
```bash
node scripts/research-format-catalog.mjs --input tmp/Prospects.csv --output-dir tmp/catalog-research-2026-11--2027-02-v9 --as-of 2026-09-09 --min-days-before 0 --date-from 2026-11-01 --date-to 2027-02-28 --limit 20 --verbose
node scripts/research-format-catalog.mjs --input tmp/Prospects.csv --output-dir tmp/catalog-research-2026-11--2027-02-v9 --resume --limit 20 --verbose
```

Use a fresh output directory for the first run. `catalog-progress.json` stores schema version 2, pipeline version 9, the input SHA-256, campaign date/window (including `date_from` and `date_to`), LLM mode, queue keys, completed rows, and next offset. Each completed format is checkpointed through a temporary file and rename; resume regenerates the CSV exports from that state. A changed input, incompatible queue, changed campaign settings, legacy pipeline version, or legacy output directory is refused so one campaign cannot mix extraction rules. `catalog.lock` prevents concurrent writers. `SIGINT`/`SIGTERM` clean up the lock; if a harder termination leaves one behind, the next invocation removes it only when its recorded PID is no longer alive and still refuses a live concurrent process. Failed-source rows are recorded as completed research attempts; resume processes subsequent rows, so retry those failures through a separately reviewed input and new output directory.

### Fetching and evidence

`enrich-format-import-queue.mjs` performs deterministic extraction and optionally semantic extraction. HTTP resources are cached by canonical URL, while crawl decisions and extracted claims remain specific to each format and target edition. A bounded best-first crawl explores relevant same-host links to depth two, canonicalizes tracking/fragment variants, deduplicates pages, and ranks course, programme, regulations and logistics pages. It reads sitemap declarations from `robots.txt`, tries `sitemap.xml` and `wp-sitemap.xml`, and expands nested indexes within fixed limits. Up to eight pages are fetched normally and eighteen for deeper research. Administration, participant/entrant lists, results, classifications, exports, race reports (`compte-rendu`) and bilan paths are excluded before download and cannot become evidence; ordinary public registration-information pages remain reachable. Cross-host redirects do not silently change the authoritative organizer. Script, style, template, navigation and footer content is removed before block extraction so URLs and JavaScript configuration cannot generate false distance claims. The HTTP reader limits response size (8 MB normally), has a 12-second timeout, and extracts text from PDFs using the existing web workspace `pdf-parse` dependency (maximum 100 pages). Thin JavaScript shells are reported through `rendering_status`; browser rendering, scanned PDFs and access challenges still require review.

All HTTP entry points, including direct MCP fetch/search and GPX downloads, use the same SSRF guard. Credentials, localhost/local suffixes, private/reserved/link-local IPv4 and IPv6 ranges, and hostnames resolving to those ranges are rejected. Redirects are followed manually within a fixed limit and every destination is revalidated before it is requested.

When both the supplied organizer URL and its secondary fallback fail, or when the supplied root is empty or resembles a parked domain, the crawler performs a bounded DuckDuckGo HTML search using the event, format, edition and location hints. A failing path is first retried at the supplied domain root; that root is accepted only when it identifies the expected event, and records `discovery_method: origin_root`. Before web search, a thin legacy landing page may also redirect research through a linked event-specific host when that destination passes identity and location checks; this route records `discovery_method: legacy_link`. Web search inspects at most eight results. A result is promoted only when it is not a known aggregator, its title identifies the event, its hostname contains a distinctive event token, it exposes at least two same-host organizer/course links, and the page contains the prospect city when one is available. This last constraint prevents a homonymous event in another department from becoming the source. An unresolved empty or parked page becomes a source error instead of a silent `needs_enrichment`. `discovery_method: web_search` and `source_quality: discovered_search_organizer` make successful recovery auditable. A missing or erroneous prospect city can still leave a recoverable source unresolved and requires review.

A secondary source can identify an organizer through an explicit official link or organizer metadata. Arbitrary sponsor links, canonical URLs and social profiles do not become proof of organizer authority. Accepted claims must cite a fetched page on the selected organizer host, contain the proposed value, match the target edition for dates, and identify the requested format for format-specific fields. A page containing explicit dates exclusively from another edition cannot verify any field for the target edition. Evidence explicitly attached to an older event edition is also rejected for non-date fields, even when the same page contains current content elsewhere; participant birth/category years are exempt from this historical-edition test. Claims from sections explicitly dedicated to previous editions, results, classifications, photos, videos or archives are rejected even when the section has no year. Claims from historical URLs are also rejected when their URL year differs from the target edition. A distance found only in a price line is rejected. A unique matching page heading may supply format context and is retained with the citation. A model-proposed alias alone cannot establish that match. Unknown hosts are only official candidates; the domain selection still warrants human review.

`catalog-research-contract.mjs` defines the field mapping and schema. `catalog-research-validation.mjs` normalizes numeric strings, calendar dates and route references, checks citations, detects conflicting accepted claims, and recomputes readiness. Control characters introduced by malformed page encodings are replaced with spaces before extraction and citation matching. An explicit weekday date in the selected organizer page title can establish the current event date even when archived articles make the full page contain several dates. An official page phrase such as `Dimanche 1er novembre`, or `1er novembre` beside an explicit edition/programme marker, may inherit the campaign edition year only when the resulting date remains inside the row's campaign bounds; provenance records this as `deterministic_inferred_year`. This inference is rejected when its evidence ties the yearless date to an explicitly older event edition, when the stated weekday contradicts the calendar date, or when the matching date occurs only in registration, bib-pickup, publication or pricing context. Explicit `rendez-vous/départ à <lieu>` wording, paired or shared `Départ et Arrivée : <lieu>` wording, an event venue introduced by `a lieu`/`aura lieu`/`se déroulera`, `<lieu>, le trail/course...` wording, and regulation wording locating the start and finish at a named place can provide deterministic location evidence. Postal complements and lowercase connectors in named venues are retained. Connector-only values such as `et` or `et d'`, a bib-pickup address, or a club/contact address cannot verify the race location, and an arrival building is not treated as a competing event location. Exact normalized citations remain preferred; a citation of at least five tokens may tolerate punctuation/entity differences only when at least 70% of its ordered three-token sequences occur on the cited page. Loose token overlap is rejected.

Conflicting claims are resolved by format specificity before being marked ambiguous. A dedicated page identified by its single distance, heading, or model-provided format label outranks an event-wide FAQ; a citation that identifies the requested format outranks a generic event statement. A generic event date does not gain format specificity merely because another block on the page contains one matching distance. Transactional registration pages have lower authority than the organizer's current course page, so a closed or stale form cannot overwrite the published format. On multi-format pages, deterministic date evidence is kept local to the requested format; dates beside a different distance, dates sourced from a URL explicitly naming another format, generic event ranges such as `les 14 et 15 novembre`, and days whose local schedule block identifies another distance are rejected instead of conflicting with the requested start. Equal-specificity disagreements still block the field, while an already accepted value keeps its strongest provenance when a later, less-specific claim repeats the same value. Compatible nested locations such as `Tourouvre` and `Tourouvre, forêt du Perche` merge in favor of the more complete description. This lets a 185 km page retain its own Friday date when the event FAQ also lists Saturday and Sunday formats. Every verified value records its specificity with value, source URL, evidence, context, method, edition and status in `field_provenance_json`. Rejected claims and fetch/LLM errors remain reviewable. A refresh clears stale verification flags. Verified organizer data can replace an unverified prospect estimate, including D+; prospect values are retained separately. If the verified distance differs from the prospect format by at least 1 km and 3%, readiness is blocked with `format_identity` until a reviewer explicitly reconciles the renamed or changed format.
...
Set `OPENAI_API_KEY` and optionally `OPENAI_ORGANIZER_IMPORT_MODEL`, `LLM_API_URL`, or `LLM_MODEL`; omit `--no-llm` to enable semantic extraction. The LLM still researches optional fields after deterministic mandatory-field extraction. Its context is bounded to 24,000 characters and assembled from whole ranked document blocks, with mandatory-field cues first and strong penalties for neighboring formats or historical sections. There is no separate paid page-mapping pass. The 60-second timeout covers both response headers and body; provider errors preserve deterministic evidence and do not abort the batch. Structured logistics are returned as JSON-encoded claim strings for strict-schema compatibility.

The local `race-research-mcp.mjs` server exposes `crawl_source`, `fetch_page`, `search_page`, `parse_gpx`, and `download_and_parse_gpx`. The default adapter starts the MCP bridge automatically. Its crawl request includes the target edition, campaign bounds, city, prospect city and country so MCP discovery applies the same homonym and edition checks as direct HTTP. A failed request closes only that client, starts a fresh bridge, and retries the current race once; a second failure falls back to direct HTTP for that race. Non-JSON diagnostics accidentally written to child stdout are ignored and reported on stderr instead of poisoning every later request. `RACE_RESEARCH_USE_MCP=0` selects direct HTTP. `crawl_transport` records the path used. MCP and HTTP use the same evidence rules and requested field set; neither is an authority. Every row prints a start marker before network or LLM work; `--verbose` additionally shows decisions without printing credentials.

### V9 robustness layers

Every selected root, including a directly supplied URL, receives an auditable identity score/status based on event name, title/content, host and location signals. `verified` pages may be authoritative; `probable` pages remain secondary, and `unknown` roots are rejected or sent through source recovery. Municipal or club domains can be verified from strong page content without weakening the rule globally. `source_identity_status`, `source_identity_score`, `source_identity_json` and `rendering_status` are preserved in checkpoints, CSV exports and the MCP path.

Pages are converted into stable structured blocks retaining URL, heading path/section, text, detected dates/distances and field cues. The single semantic call receives whole ranked blocks rather than equal arbitrary page slices; mandatory fields are prioritized with a bounded quota that reserves room for optional logistics, while neighboring-format and archive blocks are penalized. Context remains bounded to 24,000 characters and carries explicit block/source/section labels. Every semantic claim must return the exact `block_id`, source URL and an excerpt grounded inside that block; ungrounded IDs or cross-block synthesis are discarded. This preserves the one-call workflow while reducing cross-format contamination.

Claims are normalized and validated individually, then resolved together through `catalog-research-claim-ledger.mjs`; their input order cannot change the outcome. Equivalent values are clustered, support and independent source counts are retained, stronger format specificity wins, and equal-strength disagreements create a stable blocking conflict. LLM confidence and rationale remain audit-only metadata and never raise a claim's authority.

`events.csv` merges complementary event-wide values and source pages from every format instead of using the first row. Differing candidates are exported deterministically in `event_conflicts_json`, with selected support in `event_field_provenance_json`; these conflicts require review and are not permission to publish the selected representative.

### Stratified benchmark

Do not use the first priority-queue page as a generalization benchmark. `catalog-research-benchmark.mjs select` creates deterministic, event-disjoint `dev`, `regression` and `holdout` splits across source roles, distance buckets, HTTP/HTTPS, single/multi-format events, PDFs and detected hosting platforms. It writes a complete combined queue and one executable queue per split, preserving the original `format_key`; pass those files through `research-format-catalog.mjs --queue-input` instead of rebuilding prospects. Expected values remain empty until a human marks a row `reviewed`; the tool never manufactures benchmark truth.

```bash
node scripts/catalog-research-benchmark.mjs select --input tmp/Prospects.csv --output-dir tmp/catalog-benchmark-v1 --seed catalog-research-v1 --dev-size 45 --regression-size 27 --holdout-size 18
node scripts/research-format-catalog.mjs --queue-input tmp/catalog-benchmark-v1/benchmark-dev-queue.csv --output-dir tmp/catalog-benchmark-v1/dev-results --as-of 2026-09-09 --min-days-before 0 --date-from 2026-11-01 --date-to 2027-02-28 --verbose
node scripts/catalog-research-benchmark.mjs evaluate --manifest tmp/catalog-benchmark-v1/benchmark-manifest.csv --predictions tmp/catalog-benchmark-v1/dev-results/formats-wide.csv --output tmp/catalog-benchmark-v1/dev-report.json
```

The evaluator reports precision, recall and accuracy for date, location, distance and D+, plus edition, format, source and readiness outcomes. Keep holdout events out of rule design. Duplicate annotations and invalid reviewed statuses are rejected.

### Outputs, GPX and draft import

The complete pipeline writes `events.csv`, `formats.csv`, `formats-wide.csv`, `source-claims.csv`, `aid-stations.csv`, `gpx-manifest.csv`, `excluded-prospects.csv`, and `run-summary.json`. Both format CSVs retain the complete import contract. Source claims use the shared field mapping, so dates, locations and logistics retain their actual values and URLs. Per-format `evidence/` JSON snapshots store the bounded validation text, page URL/title/authority, request context, extraction response, model metadata and errors; raw multi-megabyte HTML is not duplicated in every snapshot. Downloaded GPX files are stored by SHA-256. These local artifacts remain under ignored `tmp/`.
...
<!-- NEEDS REVIEW: 2026-09-07 - Outreach behavior was not re-audited during the catalog reliability changes; the 2026-09-09 verification covers the catalog-research sections and file references. -->
...
- Source identity scoring is deliberately conservative. Atypical organizer sites can remain `probable` or `source_error`; review `source_identity_json` and correct the source rather than weakening authority globally. A `probable` page can support diagnostics but cannot verify claims.
- Benchmark manifests contain no truth until a reviewer fills expected fields and sets `annotation_status=reviewed`. Never tune rules against the holdout split and continue calling it holdout.
- Event-level conflicts in `events.csv` are review data, not permission to publish the deterministically selected representative.
...
- A previous campaign may already contain participant data fetched before pipeline version 6. Do not resume or import that directory; regenerate it with the current pipeline (v9), verify the new evidence snapshots, then remove the obsolete local artifacts according to the project's data-retention policy.
- Search recovery is deliberately conservative. A club or municipal domain can pass through strong event content, but a sparse page may remain `source_error`; review its identity signals and correct `official_website` rather than weakening source authority globally.
- A model-generated sentence assembled from facts in different paragraphs is not a citation. Fuzzy grounding tolerates typography and HTML-entity differences, not paraphrase or synthesis.
