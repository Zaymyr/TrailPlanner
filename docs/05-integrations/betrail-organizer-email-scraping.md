...
```bash
node scripts/research-format-catalog.mjs --input tmp/Prospects.csv --output-dir tmp/catalog-research-2026-11--2027-02-v7 --as-of 2026-09-09 --min-days-before 0 --date-from 2026-11-01 --date-to 2027-02-28 --limit 20 --verbose
node scripts/research-format-catalog.mjs --input tmp/Prospects.csv --output-dir tmp/catalog-research-2026-11--2027-02-v7 --resume --limit 20 --verbose
```

Use a fresh output directory for the first run. `catalog-progress.json` stores schema version 2, pipeline version 7, the input SHA-256, campaign date/window (including `date_from` and `date_to`), LLM mode, queue keys, completed rows, and next offset. Each completed format is checkpointed through a temporary file and rename; resume regenerates the CSV exports from that state. A changed input, incompatible queue, changed campaign settings, legacy pipeline version, or legacy output directory is refused so one campaign cannot mix extraction rules. `catalog.lock` prevents concurrent writers. `SIGINT`/`SIGTERM` clean up the lock; if a harder termination leaves one behind, the next invocation removes it only when its recorded PID is no longer alive and still refuses a live concurrent process. Failed-source rows are recorded as completed research attempts; resume processes subsequent rows, so retry those failures through a separately reviewed input and new output directory.

### Fetching and evidence

`enrich-format-import-queue.mjs` performs deterministic extraction and optionally semantic extraction. HTTP resources are cached by URL, while crawl decisions and extracted claims remain specific to each format and target edition. Up to eight pages are fetched normally, eighteen for deeper research; same-host course, programme, organization, regulations and logistics links are ranked along with the site's sitemap. One level of nested sitemap indexes is expanded so WordPress page sitemaps lead to actual source pages. Administration, participant/entrant lists, results, classifications, exports, race reports (`compte-rendu`) and bilan paths are excluded before download and cannot become evidence; ordinary public registration-information pages remain reachable. Script, style, template, navigation and footer content is removed before block extraction so URLs and JavaScript configuration cannot generate false distance claims. The HTTP reader limits response size (8 MB normally), has a 12-second timeout, and extracts text from PDFs using the existing web workspace `pdf-parse` dependency (maximum 100 pages). JavaScript-only content, deeper sitemap nesting, scanned PDFs and access challenges can still require review.
...
- A previous campaign may already contain participant data fetched before pipeline version 6. Do not resume or import that directory; regenerate it with the current pipeline (v7), verify the new evidence snapshots, then remove the obsolete local artifacts according to the project's data-retention policy.
- Search recovery is deliberately conservative. An organizer hosted on an unrelated club or municipal domain may remain `source_error` because the hostname does not contain a distinctive event token; review the search candidates and correct `official_website` rather than weakening source authority globally.
