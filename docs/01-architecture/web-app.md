...
- The event `Informations` editor stores the optional official website plus a structured emergency contact name and phone in `race_events.organizer_details`. These values stay behind the existing organizer membership-checked event route; no separate event column or client-side Supabase write is introduced.
...
- Keep the emergency contact at event scope as `organizer_details.emergencyContact`; mobile turns only its published phone into a `tel:` action and must not treat it as a normalized user/contact record.
