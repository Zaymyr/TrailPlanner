...
- `/organizer` lets approved organizers manage their claimed events through a modular dashboard with event synthesis, separate event and format completion surfaces, per-format tabs, ravito tables, common-vs-format JSONB detail modules, and an internal runner preview.
