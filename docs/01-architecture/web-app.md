...
For a brand-new organizer format, the add-format form may also hold a pending image and GPX file before submit. `OrganizerDashboard.tsx` parses the selected GPX in-browser to prefill distance, D+, and D- immediately, and it requires a format race date before calling the create route. The format editor now keeps race information in a tighter left grid and uses a dedicated right-side GPX rail with upload state, a larger interactive OpenStreetMap/Leaflet route map, and a denser elevation preview, so the screen avoids heavy nested-card stacking while still keeping the course media visible. After the create route returns the new `races.id`, the dashboard reuses the existing organizer image and GPX server routes to upload those files rather than writing to Storage directly from the browser.
