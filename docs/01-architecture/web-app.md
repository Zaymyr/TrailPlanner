...
For a brand-new organizer format, the add-format form may also hold a pending image and GPX file before submit. After the create route returns the new `races.id`, `OrganizerDashboard.tsx` reuses the existing organizer image and GPX server routes to upload those files rather than writing to Storage directly from the browser.
