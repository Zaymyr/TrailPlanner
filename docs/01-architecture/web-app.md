...
For a brand-new organizer format, the add-format form may also hold a pending image and GPX file before submit. `OrganizerDashboard.tsx` parses the selected GPX in-browser to prefill distance, D+, and D- immediately, and it requires a format race date before calling the create route. The format editor now keeps race information in a tighter left grid, uses a right-side file rail for GPX-first then image upload, and promotes the interactive OpenStreetMap/Leaflet route map plus the denser elevation preview into a larger block underneath, so the course preview stays the main visual focus without heavy nested-card stacking. After the create route returns the new `races.id`, the dashboard reuses the existing organizer image and GPX server routes to upload those files rather than writing to Storage directly from the browser.
