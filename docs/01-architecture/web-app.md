...
Organizer edits are draft-capable source edits for `race_events`, `races`, `race_aid_stations`, and `race_aid_station_products`. Event/race routes persist nullable organizer JSONB, geocoded metadata, images, equipment, access, schedules, GPX-derived stats, ravitos, and products behind active event membership checks. The event detail route also embeds the persisted ravito count for every format; header completion therefore remains stable when the active format tab changes instead of borrowing the selected editor's sidecar state. The dashboard shows completion independently from publication and displays read-only live/brouillon badges. Event, edition, format-tab, and module navigation changes scope immediately while the previous scope saves through a silent per-scope background queue; actions that require stored truth, such as preview or publication, still await foreground save completion. Organizer routes never mutate event or format liveness directly: `/api/organizer/publication-requests` validates event name/location/start/end dates plus at least one complete format, and `/api/admin/event-publication-requests` performs the atomic approval. Runner preview continues to hide drafts. Existing saved plans remain snapshots, while official ravito products may be refreshed for plans linked to a source race.
