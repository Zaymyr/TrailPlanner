...
The publication dialog captures the selected event id, canonical edition id, year, and effective tier when it opens, displays the event/year context, and reuses that stable snapshot for checkout. A verified admin may use the same snapshot to grant the partner edition RaceBook without Stripe; ordinary organizers never receive that action. Dated legacy/imported formats missing `edition_id` are repaired by the canonical-edition backfill rather than billed or granted from a year-only browser guess.
