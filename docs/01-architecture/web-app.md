...
The Organizer edition header reads `race_event_editions.is_visible` and exposes one edition-wide catalog visibility switch in its expandable summary. Every format row independently switches `racebook_preview_is_visible`, including before purchase or publication: turning it off removes that format from the organizer demo and clears its runner publication flag, while turning it on restores only private preview eligibility. `POST /api/organizer/editions/[id]/publication` is the distinct action that atomically publishes all selected, complete formats after organizer/admin, edition and entitlement checks. Edition and event deletion use explicit, touch-sized destructive actions separated from their selectors. `PATCH /api/organizer/editions/[id]` hides or restores complete course rows after active membership validation; hiding always forces attached Racebooks off. `DELETE` requires the client confirmation dialog to match the selected year, invokes the atomic service-only deletion RPC, cleans up format GPX/images, and returns the remaining year the dashboard should select.
...
- `/organizer` lets active event members maintain catalog-visible events, canonical `race_event_editions` ranges, and attached formats. Its compact summary is closed by default; the offer and publication CTA remain visible, while expansion reveals the year selector beside `Créer une nouvelle édition` and the explicit destructive actions. A labelled event filter appears above eight selector entries and retains the active event while filtering. The compatibility `/api/organizer/edition-requests` URL creates the edition either empty or with cloned formats and no longer writes the retired review table. Catalog liveness stays read-only. Every format switch controls only private demo selection and is available without an offer; disabling it also unpublishes that format. The separate publication CTA publishes selected formats only after Essential, Complete or Signature is active. The offer badge and payment/gift status remain adjacent; admin and legacy-admin grants stay auditable as complimentary access.
...
- Keep private-demo switch saves scoped to the switched format. Do not foreground-save an unrelated active draft before showing or masking another format; runner publication remains one explicit edition action.
