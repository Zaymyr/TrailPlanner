...
The Organizer edition header reads `race_event_editions.is_visible` and exposes one edition-wide visibility switch in its expandable summary. Edition and event deletion use explicit, touch-sized destructive actions separated from their selectors. `PATCH /api/organizer/editions/[id]` hides or restores complete course rows after active membership validation; hiding always forces attached Racebooks off. `DELETE` requires the client confirmation dialog to match the selected year, invokes the atomic service-only deletion RPC, cleans up format GPX/images, and returns the remaining year the dashboard should select.
