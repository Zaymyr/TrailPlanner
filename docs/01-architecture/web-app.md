...
- `/organizer` lets approved organizers manage their claimed events through a modular dashboard with event synthesis, one tabbed completion surface whose first tab is the event and whose following tabs are formats, ravito tables, common-vs-format JSONB detail modules, and an internal runner preview.
