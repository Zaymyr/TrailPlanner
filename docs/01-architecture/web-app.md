...
OpenAI receives only incompatible applicable claims and uses strict Structured Outputs. It can return an existing applicable `claimId` or `uncertain`; it cannot return a value or select a previous-edition reference claim. Distance concordance uses `max(0.5 km, 2%)`, and D+/D- use `max(100 m, 8%)`. Only a high-confidence, conflict-free fill of an empty field may be preselected.
...
For non-provider sites, the general URL remains the source for event-level facts and common logistics and can also yield structured format blocks. With no explicit format URL, the importer discovers a bounded set of hint-matched same-origin pages. Explicit URLs are authoritative: general-page tabs or embedded routes are then excluded from the format set and cannot filter the explicit pages. If the general URL is repeated in that explicit list because it is itself a format page, it is fetched once and parsed as an explicit format too. When repeated event-level JSON-LD conflicts with a precise heading/course block on one explicit page, the visible page-specific identity wins. A route embedded inside an explicit page is a page-local anchor: nearby declared metrics may enrich it, while relay legs and incidental kilometer mentions incompatible with that route are discarded instead of becoming extra formats. Extraction otherwise combines JSON-LD, accessible tabs, embedded GeoJSON, headings, and named regulation prose. Candidate consolidation requires compatible normalized identity evidence. Anonymous same-distance detections remain separate for the admin, and proximity alone never attaches a GPX to a named candidate across pages.
...
- Explicit format URLs are an authority boundary. A route embedded on the general event page must not reduce their candidate set, and generic event JSON-LD repeated across them must not replace their distinct visible headings.
