...
- Admin organizer assignment and autocomplete must resolve e-mails through the server-only Supabase Auth Admin API. Autocomplete requires at least two characters and returns only a bounded matching subset; submission still performs an exact case-insensitive lookup. New-account invitations must redirect to the allowlisted `/reset-password` flow, which sends invite sessions to `/organizer` after password creation. Never return the complete Auth user list or service credential.
