...
- `/organizers` creates a new non-live event through `POST /api/organizer/events` and immediately creates its active owner membership. It does not claim existing catalog events. With an official URL, the page redirects to `/organizer` with the new event selected and automatically opens its website-import analysis.
- `/organizer` lets organizers with an active membership manage their events through a modular dashboard with compact event synthesis, an event-level year selector above the progress rows, a sibling `Nouvelle édition` validation request action in that same header area, one tabbed completion surface whose first tab is the event and whose following tabs are organizer format-series tabs keyed by `races.series_name`, planner-style ravito cards, common-vs-format JSONB detail modules, and an internal runner preview. The selected year resolves the active `races` edition per format through `races.edition_group_id`. New yearly editions must first go through `race_event_edition_requests` review: pending requests appear immediately in the organizer year selector as disabled options, and admin approval then clones the source-year format rows so the year becomes selectable and editable. `OrganizerDashboard.tsx` stays the client-state orchestrator while route-local dashboard components own the reusable shell, controls, editors, ravito/product blocks, and runner preview.
...
- `/api/admin/organizer-claims` and the admin "Organisateurs" tab handle claim approval, rejection, membership revocation, and yearly edition-request review. The GET route now enriches those rows with organizer identity from `user_profiles` and Supabase auth so the admin tab can show names/emails instead of raw ids, while still separating the review queue from the active-access section. That enrichment is best-effort: if identity or yearly-edition reads fail, or if one auth user carries a blank/malformed email string, the route should still return the base claim and membership payload with fallback labels.
...
Relative to Tuesday, July 21, 2026, the organizer write surface is now edition-aware: any selected edition whose reference date is more than 14 days in the past becomes read-only in the dashboard, and the corresponding event/race mutation routes still reject direct writes for that edition.
