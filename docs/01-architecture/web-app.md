...
Enrichment converts website, structured data, GPX, current Organizer data, previous-edition references, and paginated PDF findings into typed source claims. The review displays current value, alternatives, source URL or document/page, evidence, confidence, conflicts, and missing fields for every confirmed format. New formats may stay incomplete hidden drafts. OCR for images and scanned PDFs remains pending.
...
For non-provider sites, the general URL remains the source for event-level facts and common logistics and can also yield structured format blocks. With no explicit format URL, the importer discovers a bounded set of hint-matched same-origin pages; explicit URLs remain authoritative. Extraction combines JSON-LD, accessible tabs, embedded GeoJSON, headings, and named regulation prose while filtering incidental kilometer mentions. Candidate consolidation requires compatible normalized identity evidence. Anonymous same-distance detections remain separate for the admin, and proximity alone never attaches a GPX to a named candidate.
