...
Cold-start and post-auth navigation resolve onboarding first, then open the Courses catalog without preloading the Plans screen.
...
The RaceBook starts its full data request and the lightweight `/api/racebook-sponsors` request in parallel. Its initial full-height loading composition reserves two stacked sponsor panels immediately, shows the returned logos as soon as the lightweight response arrives, and uses an animated runner on a determinate-looking progress track to make the wait legible. The track advances toward a guarded pre-completion ceiling, then visibly reaches 100% before the content replaces it. The sponsor area occupies roughly one third of the available viewport and collapses back to the ordinary loader when the response is empty or fails. When one or two loading sponsors exist, mobile preloads their logos, starts a 2.5-second minimum presentation only after that composition is ready, and opens the content when both the data and minimum duration are complete. Pull-to-refresh reloads only RaceBook data so the sponsor interstitial is not replayed.
...
- Keep sponsor requests server-mediated and edition-scoped. Reserve the full loading composition before the lightweight sponsor response so late logos do not shift the page, but remove its empty sponsor area once the lookup settles without placements. Never expose direct sponsor table access or the destination website URL, never replay the 2.5-second sponsor gate on refresh, and keep reduced-motion users on the manual banner list.
