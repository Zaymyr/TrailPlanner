...
- its public event relation uses an inner join filtered to `races.is_live = true`, so masked/private formats do not reach ordinary runners; after resolving active organizer memberships, a second event-id-bounded relation loads managed formats for private previews, then the presentation filter removes every format whose `racebook_preview_is_visible` is false;
- masked formats are absent from the mobile catalog for runners and organizers; organizer-owned private formats remain listed with a lightly dimmed but functional RaceBook preview action, while public formats use the normal runner presentation;
...
- The mobile catalog has an explicit runner-facing organizer contract for `race_events.organizer_details` / `races.organizer_details`: use `apps/mobile/lib/racebook.ts` to keep gating, parsing, and read-only composition aligned. `racebook_preview_is_visible` must be true for both paths. Ordinary runners additionally require course `is_live` and `racebook_is_live`; an active event organizer resolved through `race_event_organizers` may preview before publication, but cannot open a format explicitly masked from the private demo. Both paths still require meaningful non-ravito organizer content; aid stations alone are not enough.
