...
The RaceBook starts its full data request and the lightweight `/api/racebook-sponsors` request in parallel. Its dedicated loading composition keeps the native back/title header but temporarily hides feedback and the bottom tab bar. A localized preparation title, thin progress trail, and unframed runner form one compact group above a single sponsor panel; the panel reserves two vertically stacked slots separated by one subtle divider, shows returned logos as soon as the lightweight response arrives, and occupies roughly one third of the available viewport. The track advances toward a guarded pre-completion ceiling, then visibly reaches 100% before the content replaces it. An empty or failed sponsor response removes the reserved panel and returns to the ordinary progress loader. When one or two loading sponsors exist, mobile preloads their logos, starts a 2.5-second minimum presentation only after that composition is ready, and opens the content when both the data and minimum duration are complete. Pull-to-refresh reloads only RaceBook data so the sponsor interstitial is not replayed.
