...
- Keep the mobile Racebook read-only. It consumes published organizer details plus live ravito source data, but it should open only when non-ravito organizer content is actually published; it must not import organizer dashboard mutation logic or admin routes. The runner-facing screen now uses the format race date in the header, surfaces format `elevation_loss_m` alongside distance and D+, extracts `services.lastMinuteMessage` into a dedicated alert card above the local tabs when present, keeps the remaining service copy inside the Profile tab, renders start and bib fields as two-column table-like rows, keeps each gear status badge inline with its item label while allowing wrap on tight mobile widths, and splits each ravito card into a content column plus a right metrics column for km, D+, D-, and cutoff time.
