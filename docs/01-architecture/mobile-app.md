...
Race detail stacks use minimal iOS back indicators so dynamic route segments such as `[id]/racebook` are never exposed as user-facing copy. Every RaceBook exit dismisses to the Courses catalog instead of following arbitrary navigation history; Expo Router falls back to replacing the current route when a direct opening has no matching catalog entry.
...
- Keep the tab navigator on history-based back behavior for ordinary hidden detail screens. Switching it back to `initialRoute` changes their back semantics globally; RaceBook owns its narrower Android-back override and dismisses specifically to `catalog`.
