...
The login route follows the signup scroll/keyboard-avoidance pattern so its password, account-creation, and platform sign-in actions remain reachable on compact iPhones and at enlarged text sizes. Modal sheets isolate VoiceOver focus with `accessibilityViewIsModal`, identify their titles as headers, and give icon-only dismiss actions an explicit label and at least a 44-point touch target or equivalent hit slop.
