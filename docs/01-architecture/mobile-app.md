...
- The mobile Racebook now complements organizer copy with stored route data when available: the `Profil` tab starts with a lightweight native route sketch sourced from GPX points, and the `Ravito` tab starts with the course elevation profile before the aid-station list.
