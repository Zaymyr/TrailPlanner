...
- The mobile Racebook uses a compact identity card for event/format identity, event date range, optional distinct format date, location, and runner information; distance, D+, D-, and start-time metric pills are intentionally omitted from this synthesis. Its flexible metadata row uses calendar/location icons, dot separators, and compact `Solo` and/or `Relais` badges, with mixed formats rendered as two separate badges. Its four permanent read-only tabs are `Matériel`, `Dossard`, `Course`, and `Accès`, plus a fifth `Services` tab only when event service details exist. `Course` owns the start time and the explicitly labeled finish cutoff time, then separates its longer content into `Tracé` (interactive GPX map plus elevation profile), `Ravitos`, and a conditional `Relais` sub-tab. On `Tracé`, `SAS`, and `Ravitos`, those two times collapse into the same compact summary and course constraints stay behind one expandable count so the selected content begins near its tab. SAS rows omit a redundant outer section card and expose sequence, name, start time, and runner criterion directly. The ravito list uses a continuous vertical chronology: each station card keeps its cumulative kilometer, services and optional cutoff, while the always-visible connector before it states the distance, D+ and D- from the previous station or from the start. Only stations with products or notes are expandable, and only one opens at a time. Service icons are descriptive, not buttons. `Accès` orders access notes and road restrictions in an amber `À retenir` block, then locations/maps, then a `Venir sur place` card whose parking and shuttle rows start collapsed with two-line summaries; expanding the shuttle row separates its schedule in a clock treatment. Parking, shuttles, road restrictions, and map content are runner-visible only while their corresponding format-level access flag is enabled, even if a saved value still exists. When published, the official website, Instagram, and Facebook links appear as a compact icon-only outlined group beside the race identity, with Instagram and Facebook below the website icon; accessibility labels retain their meaning without visible text. A divider separates the emergency contact, which keeps `Urgence - nom - téléphone` on one line alongside a localized outlined call action; the full row opens the platform phone app through `tel:`. The native header keeps feedback only. Pulling down anywhere on the screen reloads the Racebook, profile, and route data while preserving the last successful content if that refresh fails.
