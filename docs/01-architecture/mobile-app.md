...
- The mobile Racebook groups its read-only content into `Général`, `Matériel`, `Dossard`, `Course`, and `Accès`. `Général` exposes event identity, dates, runner information, and services; `Dossard` contains pickup details; `Course` combines the interactive GPX map, elevation profile, and aid stations; `Accès` contains start/finish locations, parking, shuttles, restrictions, and access notes.
...
- Keep the mobile Racebook read-only. It consumes published organizer details plus live ravito source data, but it should open only when non-ravito organizer content is actually published; it must not import organizer dashboard mutation logic or admin routes. The runner-facing screen uses the format race date in the header, surfaces format `elevation_loss_m` alongside distance and D+, extracts the event equipment weather plan into a dedicated compact weather alert card above the last-minute message when the plan is `cold` or `heat`, keeps `services.lastMinuteMessage` in its own compact alert card below that warning, renders alert title and message inline on the same text row, and organizes content into `Général`, `Matériel`, `Dossard`, `Course`, and `Accès`. The `Course` tab starts with the interactive GPX map and elevation profile, then lists ravitos; `Accès` owns start/finish locations and related logistics. Start and bib fields remain responsive table-like rows whose divider expands to the value while long linked locations retain enough width to wrap cleanly. Equipment remains ordered as active required items first, then active recommended items, then weather-muted inactive items, with inline right-aligned status badges and icon-only cold/heat markers. Ravito cards retain their compact right metrics column whose km, D+, D-, and cutoff labels stay inline with their values.
