...
The normalized format details include `gpxDisplay.showRoute` and `gpxDisplay.showElevationProfile`, each defaulting to `true`. The `Tracé` view independently omits the map or elevation profile according to those organizer choices and renders no GPX empty state when both are intentionally disabled.

The mobile `Tracé` view gives route geometry its own interaction hierarchy. The Leaflet map and elevation profile each expose a 44-point expand action; the map opens edge-to-edge below a native safe-area-aware close bar, while the profile opens as a rotated landscape canvas without changing the app-wide portrait orientation lock. Published ravitos are projected from their cumulative kilometer onto both visuals, stay unlabeled until selected, and reveal their name, services, cutoff and organizer note on demand. Map markers retain a compact visual point but use a larger transparent touch target; their popup emphasizes cumulative distance and D+ before secondary details. Projection is limited to the measured GPX/profile extent so a station beyond an incomplete trace is never pinned falsely to its endpoint. The profile chooses the most complete elevation-bearing source, adds altitude/distance guides, and warns when its measured extent is materially shorter than the advertised race distance. Pressing or sliding on the chart moves a persistent cursor; the three cards immediately below the chart display measured cumulative distance, D+ and D- instead of duplicating static course totals. Only a narrow proximity around a ravito snaps the cursor to that station and replaces those cards with its details; cumulative distance and D+ remain prominent statistic blocks instead of being buried in metadata. Sub-metre elevation noise is ignored, and screen readers can move the same cursor through adjustable actions. The landscape variant allocates extra height to the chart and omits the already-seen coverage notice so the graph uses the short side of the phone more fully.
...
On cold start and after authentication, sessions that do not require onboarding open on the `catalog` Courses tab by default. The tab shell in `apps/mobile/app/(app)/_layout.tsx` also registers hidden detail routes such as `race/[id]/racebook` explicitly so Expo Router does not surface them as bottom-tab destinations while keeping normal pushed navigation behavior. The tabs use history-based back behavior for ordinary hidden detail routes. RaceBook is the deliberate exception: its hero return action, contextual Courses action, and Android hardware back all replace the active route with `catalog` so opening several formats can never reveal a previously viewed RaceBook.
...
Race detail stacks use minimal iOS back indicators so dynamic route segments such as `[id]/racebook` are never exposed as user-facing copy. Every RaceBook exit replaces the active route with the Courses catalog instead of following arbitrary navigation history, including after a direct opening.
...
- Keep the tab navigator on history-based back behavior for ordinary hidden detail screens. Switching it back to `initialRoute` changes their back semantics globally; RaceBook owns its narrower Android-back override and replaces specifically with `catalog`.
...
- Keep GPX display preferences presentation-only and backward compatible. Missing or malformed `gpxDisplay` values must show both visuals, while explicit `false` values hide only their matching RaceBook card.
- Keep interactive course ravito markers derived from published station kilometers and measured route/profile distances. Never clamp an out-of-range station to the final GPX point, and keep marker hover/selection, the below-chart distance/D+/D- readout, fullscreen state, and rotated landscape presentation local to the screen rather than persisted or added to analytics. Cumulative cursor values describe only the measured profile portion and must not be scaled to an advertised full-course total when the GPX is incomplete.
