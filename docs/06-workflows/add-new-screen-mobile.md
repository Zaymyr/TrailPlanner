...
- RaceBook sponsors come from the lightweight web API bridge in parallel with the main snapshot. The initial screen reserves two large stacked sponsor panels (roughly one third of the viewport) below the runner progress track, fills them as soon as the lookup returns, and removes the reserved area if the lookup settles empty. Let the progress stop below completion while work remains and visibly reach 100% before replacing the loader. Keep the 2.5-second gate conditional on ready loading sponsors, skip it on pull-to-refresh, and respect reduced motion in the compact banner.
