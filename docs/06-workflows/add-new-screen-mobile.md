...
- Keep the `Tracé` visuals focused and interactive: use `RacebookLeafletMap` and `RacebookElevationProfile`, project ravitos by cumulative kilometer without permanent labels, reveal details only after selection, and provide separate 44-point fullscreen controls. Map ravitos keep a small visible point but must use a larger invisible touch target; their detail prioritizes cumulative distance and D+. Pressing or sliding outside the profile's narrow ravito snap radius must move the cursor and update three cards below the graph with measured cumulative distance, D+ and D-. Entering that narrow radius snaps to the station and temporarily replaces those cards with its details while preserving prominent cumulative distance and D+ values; keep the same exploration available through screen-reader adjustable actions. The full map must keep its close action below the native safe area; the full profile may rotate its canvas to landscape while the application remains portrait-locked, reserve more of the short side for the graph, and keep the same below-graph dynamic content. If the measured GPX/profile is materially shorter than the advertised distance, show that limitation in the inline card instead of stretching the data, scaling cursor totals to the full course, or placing later ravitos at the endpoint.
...
- Do not switch the tab shell back to `backBehavior: 'initialRoute'` for hidden child flows unless you explicitly want Android hardware back to jump to the default tab instead of the previous screen. Keep route-specific exceptions, such as RaceBook's deterministic Courses exit, local to that route.
