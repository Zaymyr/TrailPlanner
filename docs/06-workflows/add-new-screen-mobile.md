...
- RaceBook edition branding travels in that same additive response. Resolve it through `packages/design-system/src/branding.ts`, prefetch the published logo before visible progress, use `contain` and a silent image-error fallback, and keep semantic red/orange/blue states plus native navigation outside organizer control.
