...
- The Racebook publication requirement above applies to ordinary runners. After verifying `race_event_organizers`, Courses keeps managed private formats for preview but removes every masked format (`racebook_preview_is_visible = false`) from cards, counts, deep links, and format sheets. A private format keeps a dimmed functional preview action. The direct screen still verifies membership, and aid stations alone do not unlock it.
...
For Courses visibility work, verify both roles across all three states: runners see neither masked nor private formats and see public formats normally; active organizers also see no masked formats, see private formats with a dimmed functional RaceBook action, and see public formats normally. Keep the organizer merge bounded by membership event ids, filter masked rows after merging, and deduplicate stable race ids.
