...
- Compact detail routes under an existing stack, such as `race/[id]/racebook`, can keep a route-local tab bar/state machine. Keep the Racebook entry point hidden until the course format is live, `racebook_is_live` is true, and real organizer content exists; aid stations alone should not unlock it. Preserve the existing identity card, including compact `Solo` and/or `Relais` badges immediately before the `Jour de course` / `Race day` row, two separate badges for mixed formats, and its emphasized localized format-date row when that date differs from the event start date, alerts, four permanent tabs, conditional Services tab, responsive location, route, ravito, and pull-to-refresh behavior. In `Dossard`, render each pickup address directly without a repeated numbered location heading. In `Accès`, honor every format-level enabled flag before exposing saved parking, shuttle, road-restriction, or map content. Keep feedback in the native header; place the conditional official-site action in the identity card's content-sized right-hand tile without a forced-height wrapper, and keep the compact full-width emergency row directly below the identity header on one `Urgence - nom - téléphone` line. Use the display-normalized phone without separators for its `tel:` URL.
- Relay segments belong in the existing Racebook `Course` tab. Derive them from published relay points instead of creating another route or treating handovers as nutrition stations.
