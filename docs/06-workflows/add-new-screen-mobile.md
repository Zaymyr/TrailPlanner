...
- Keep native back labels minimal on dynamic routes so route templates never appear in the header. A RaceBook must route its hero return action, contextual Courses action, and Android hardware back through the same `dismissTo('/(app)/catalog')` exit so a previously opened RaceBook cannot reappear; direct openings remain covered by Expo Router's replace fallback.
