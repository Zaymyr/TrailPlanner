...
- The Racebook publication requirement above applies to ordinary runners. After verifying `race_event_organizers`, Courses keeps every managed event/format regardless of runner catalog visibility. A masked format is dimmed without a RaceBook action; a private format keeps a dimmed functional preview action. The direct screen still verifies membership, and aid stations alone do not unlock it.
...
For Courses visibility work, verify both roles across all three states: runners see neither masked nor private formats and see public formats normally; active organizers see masked formats dimmed without RaceBook, private formats with a dimmed functional RaceBook action, and public formats normally. Keep the organizer merge bounded by membership event ids, free of runner-facing visibility filters, and deduplicate stable race ids.
