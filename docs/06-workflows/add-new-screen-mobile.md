...
- Compact detail routes under an existing stack, such as `race/[id]/racebook`, can keep a route-local tab bar/state machine instead of introducing a shared navigation primitive when the screen is read-only and scoped to one flow. Keep the entry point hidden until the format is live and has real organizer content; aid-station rows alone should not unlock it. For Racebook specifically, keep the header aligned with the format date and race metrics, lift any organizer last-minute message into its own compact alert card above the tabs, render the alert title and message inline on the same text row, keep remaining service copy in Profile, start `Profil` with an interactive Leaflet map when GPX points are available, render start and bib data in responsive table-like label/divider/value rows so short values do not reserve half the row and long locations wrap cleanly, sort gear rows as active required first, active recommended second, and inactive weather-muted items last, keep gear status badges inline and right-aligned on the same row as the item label, remove bullet dots from gear rows, add icon-only inline cold/heat markers for weather-tagged gear, start `Ravito` with the course elevation profile when available, and present ravitos as a two-column layout with a compact right column where km, D+, D-, and cutoff labels stay beside their values.
