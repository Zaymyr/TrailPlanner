...
- Compact detail routes under an existing stack, such as `race/[id]/racebook`, can keep a route-local tab bar/state machine instead of introducing a shared navigation primitive when the screen is read-only and scoped to one flow. Keep the entry point hidden until the format is live and has real organizer content; aid-station rows alone should not unlock it. For Racebook specifically, keep the header aligned with the format date and race metrics, lift any organizer last-minute message into its own compact alert card above the tabs, and render the alert title and message inline on the same text row. Keep the five route-local tabs organized as `Général` for event identity/dates, runner information, and services; `Matériel` for equipment; `Dossard` for pickup details; `Course` for the interactive GPX map, elevation profile, and ravitos; and `Accès` for start/finish locations, parking, shuttles, restrictions, and access notes. Render event, start, and bib data in responsive table-like label/divider/value rows so short values do not reserve half the row and long locations wrap cleanly. Sort gear rows as active required first, active recommended second, and inactive weather-muted items last; keep status badges inline and right-aligned, omit bullet dots, and retain icon-only cold/heat markers. Present ravitos as a two-column layout with a compact right column where km, D+, D-, and cutoff labels stay beside their values.
