...
The format identity editor now uses a desktop two-column layout: left column for race information and image, right column for GPX upload plus preview blocks. That GPX column shows the upload state first, then an interactive route map and the elevation profile underneath whenever preview coordinates are available.
