...
Detections from multiple pages are merged field by field instead of replacing one another wholesale. Candidates from JSON-LD, headings, and prose are considered the same format when their normalized names match after removing embedded distance/D+ text, or when compatible-distance labels clearly contain one another. The displayed name removes generic prefixes such as `Format :` and trailing distance/D+ metadata; if that leaves only a generic label, it falls back to the parsed distance instead of inventing a course name. The consolidated candidate keeps the cleaner human name, unions complementary ravitos by distance only when their sources have equal confidence, prefers dedicated headings/regulations over loose line detections, preserves the best GPX/source fields, and recalculates its assessment only after GPX hydration. The current edition is preferred when an older parcours page conflicts with a newer regulation, and the recap keeps an explicit warning about discarded years. Ravitos mentioned in named regulation clauses are assigned only to that format. GPX links are detected both from `.gpx` URLs and from explicit `GPX` anchor labels, which supports opaque download URLs such as Odoo `/web/content/...`; each detected format can therefore hydrate its own reliable GPX, distance, elevation, and waypoints. Values are never invented: if no reliable source or GPX exposes D+, it remains in `missingFields` and creation stays unavailable until the organizer completes it. The final preview is sorted by descending assessment score, then coverage, while preserving the prior stable order for ties.
