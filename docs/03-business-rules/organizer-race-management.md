...
- Les documents sélectionnés par `Importer les informations` sont envoyés en multipart et les PDF contenant du texte sont extraits temporairement côté serveur. Les lignes reconnues produisent des observations à confirmer pour les métriques, départs, dossards, barrières, ravitos, matériel, secours et suivi live ; elles sont rattachées à un format connu seulement par nom ou distance compatible, puis comparées aux métriques déjà connues du format. Les résultats sont marqués concordants, en conflit ou non vérifiables ; les images et PDF sans texte restent en `ocr-pending`, et aucune observation n'est écrite automatiquement dans une course.
