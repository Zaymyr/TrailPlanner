...
The format identity editor now uses a desktop two-column layout with a flatter hierarchy: a compact information column on the left and a dedicated file side rail on the right. That right rail keeps only the GPX upload first and the image upload second, while the course preview moves into a larger full-width block underneath: the elevation profile appears first for a quick read, and the interactive route map sits below it as the main wide visual focus whenever GPX coordinates are available.
