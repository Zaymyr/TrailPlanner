...
- Keep direct organizer assignment and e-mail autocomplete admin-only and server-side. The browser may receive only the bounded matches for its current query, never the Supabase service credential or the complete Auth user list. A missing e-mail must remain manually enterable and be returned as a confirmable not-found result; account invitation/creation happens only after the admin confirms. Keep `/reset-password` in Supabase's allowed redirect URLs so the invite can establish a password/session before opening the organizer dashboard. Assignment must not implicitly publish or unpublish the event or its Racebook formats.
