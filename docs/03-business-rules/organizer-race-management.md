...
The admin publication tab no longer controls RaceBook visibility. Visibility remains an organizer-owned, format-scoped action. Its commercial area records Essentiel, Complet, or Signature bank transfers with payment date, actual EUR HT/TVA, calculated TTC, and an optional PDF. The organizer summary displays the effective pack, Stripe/Virement, and payment date. `Actions > Factures` lists every edition purchase for the selected event; all active event members may download an available invoice, while a missing manual PDF is shown as pending.

The same identified Supabase account always sees its managed events and every attached format in the mobile Courses catalog, independently of the runner-facing catalog toggle. Mobile resolves this exception from the active `race_event_organizers` membership created for the account e-mail; revoked memberships lose it immediately. A masked format remains listed with a lightly dimmed row but has no RaceBook action. A private format remains normally listed and its lightly dimmed RaceBook action still opens the organizer preview. A public format uses the ordinary runner presentation. This organizer-only view does not change `races.is_live` or `races.racebook_is_live` and does not expose drafts to ordinary runners.
...
No mobile organizer editor exists in v1. Ordinary runners can consume organizer details through the read-only `race/[id]/racebook` screen only when the catalog format is live, `races.racebook_is_live = true`, and there is meaningful non-ravito organizer content. Active event organizers keep every managed event and format in Courses even when the event or format is outside runner discovery. They may open the same runner-facing screen for a private preview-selected format; a masked format stays visible as a dimmed course row but exposes no RaceBook entry point. Aid stations by themselves must not surface the Racebook entry point. The screen never grants edit or admin powers.
...
- The admin publication manager exposes four distinct origins: Admin, Paiement Stripe, Paiement par virement, and Offert. Admin/Offert are editable grants; paid origins require a matching valid ledger path and never fabricate financial history.
...
- Keep masked formats absent from runner discovery but present in both the authorized web workspace and the organizer's mobile Courses view. Their grey treatment and explicit public-visibility label are status cues, not a relaxation of the runner access gate. Private formats keep an enabled but lightly grey RaceBook action for organizer preview.
