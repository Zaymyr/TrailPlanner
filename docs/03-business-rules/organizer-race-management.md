...
The admin Organizer area is split into `Publier le RaceBook` and `Accès organisateurs`. The publication-rights list can be searched by event name, location, or format name, remains combinable with the offer filter, and paginates the filtered result by ten events. In the access tab, both the e-mail and event are searchable comboboxes whose typed query and selected value share one field. Event matching covers name, location, and date. From two e-mail characters, the protected route returns at most eight matching Supabase Auth accounts; it never exposes the complete Auth directory. Submission still resolves an exact case-insensitive Auth match. If none exists, the browser shows an explicit cancel/create confirmation; only the confirmed second request creates the Auth account through the server-side invite endpoint, sends the Supabase invitation, and continues the assignment. Existing, newly invited, and reactivated accounts receive an active `organizer` membership while event/format publication state remains unchanged. The delegated organizer can edit every edition and format under the event, but only an `owner` or trusted admin can permanently delete the event.
