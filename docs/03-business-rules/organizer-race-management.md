...
related_files:
  ...
  - apps/web/components/MobileAppPrompt.tsx
  - apps/web/lib/spotlight-overlay.ts
  - apps/web/lib/spotlight-overlay.test.ts
...
## Structured RaceBook content

The edition owns nearby services; each format owns start waves and podium programme rows. All three editors load their collection only when opened and autosave with atomic replacement endpoints. Autosaves serialize revisions per editor scope. Active event membership and the selected module authorize draft writes at every tier; Essential, Complete, and Signature determine which populated sections may become runner-visible. Relay points and official products can therefore remain saved Signature drafts on a lower offer.

The combined format module is **Départ, ravitos & relais**. Its common departure and arrival cards sit above the `SAS`, `Ravitos` and conditional `Relais` tabs. The common departure is a native minute-precision time field. Single legacy clock values are normalized non-destructively to `HH:mm`; ambiguous multi-start or prose values remain stored and visible in a warning until the organizer chooses one common time or models each departure as a SAS. It exposes native SAS times and conditional criterion bounds, the existing ravito type/altitude fields, relay handover/cutoff/notes, and formerly hidden schedule constraint notes. **Podiums & récompenses** is a separate format module. **Services & alentours** is edition-scoped and keeps legacy text visible in a non-destructive conversion accordion.
...
Information and Course/GPX are permanent. Other edition or format tiles are `active`, `inactive`, or editable `draftOnly` from the shared module catalog. The section chooser uses normal switches for every tier and only one discreet minimum-offer label for draft-only sections. Switches remain local until one explicit save; disabled modules disappear from navigation and completion only after the confirmed response, while their data remains stored.
...
- Organizer details: nullable JSONB on `race_events`, `races`, and `race_aid_stations` for progressive dashboard fields that do not yet need normalized tables. Edition start/end dates are normalized in `race_event_editions`; the current range is mirrored into legacy event fields for mobile/catalog compatibility. Event details keep fields such as `officialWebsiteUrl`, `instagramUrl`, `facebookUrl`, the structured `emergencyContact` name/phone, equipment, and geocoded location metadata, while format websites remain on `races.external_site_url`. Mobile opens these validated links through one runner-facing handler that preserves telephone `+` prefixes and reports an opening failure instead of silently discarding it.
...
When an unauthenticated prospect arrives through the organizer landing, the creation page sends a validated organizer-only `next` destination through sign-in, sign-up, and OAuth. After successful auth the prospect returns to `/organizers` with only the supported UTM values, rather than being diverted to the runner planner. A protected Organizer dashboard visit similarly returns to the exact `/organizer` dashboard path; its query values are discarded. Successful creation records a consent-gated acquisition event before the existing dashboard redirect. Successful organizer content saves also emit consent-gated analytics for the event or format scope. The payload is deliberately value-free: it records technical ids, edition year, days remaining before the relevant start date, save mode, and allowlisted field categories only. Background navigation saves are distinguished from deliberate foreground saves, and an aid-station replacement is one `aid_stations` modification. These analytics do not create runner announcements and do not replace Supabase as the source of truth for persisted content or actor authorization.
...
The admin Organizer area is split into `Publier le RaceBook` and `Accès organisateurs`. The publication-rights list can be searched by event name, location, or format name, remains combinable with the offer filter, and paginates the filtered result by ten events. In the access tab, both the e-mail and event are searchable comboboxes whose typed query and selected value share one field. Their complete assignment card sits above the following active-access card so the shared backdrop blur cannot trap either suggestion panel in a lower sibling stacking context. Event matching covers name, location, and date. From two e-mail characters, the protected route returns at most eight matching Supabase Auth accounts; it never exposes the complete Auth directory. Submission still resolves an exact case-insensitive Auth match. If none exists, the browser shows an explicit cancel/create confirmation; only the confirmed second request creates the Auth account through the server-side invite endpoint, passing `/reset-password` in the `redirect_to` query parameter and the canonical race name in `data.event_name`, then creates membership and sends the Supabase invitation. The invite link lets the user choose a password, persists the resulting session, and opens `/organizer`. For an existing account, a successfully created or reactivated membership triggers one transactional Resend notice naming the event and opening password sign-in with a validated return to `/organizer?eventId=...`; it is sent after persistence and an e-mail failure does not revoke access. Existing, newly invited, and reactivated accounts receive an active `organizer` membership while event/format publication state remains unchanged. The delegated organizer can edit every edition and format under the event, but only an `owner` or trusted admin can permanently delete the event.
...
- enter optional official website, Instagram, and Facebook URLs plus an event emergency contact name and phone from the `Informations` tab; links pasted without a protocol are normalized to HTTPS when they contain a valid domain, invalid/non-HTTP(S) values are rejected, and all accepted values are saved in event-level `organizer_details`;
- edit existing race formats under the event, including format-specific `races.organizer_details`;
- add a draft format attached to the selected edition, inheriting its start date and effective event location unless explicit in-range date or location overrides are enabled, and supply the visible official source required for publication;
...
- expand `Gérer la visibilité`, closed by default, to choose one explicit state per format: `Masqué`, `Privé`, or `Public`;
- permanently delete the selected event from the secondary `Actions` menu, but only after typing the exact word `Supprimer` in the confirmation dialog; the server restricts this action to the active owner membership (or a trusted admin), removes event-owned formats and Storage assets, and leaves saved runner plans detached from their deleted source formats;
...
- with Signature, prepare, preview, save, reset, and explicitly publish one edition-wide RaceBook identity with primary/accent colors and an edition logo rendered in the runner loading and identity hero;

The dashboard is organized as a task-first header plus one tabbed completion surface. The header shows the active event and edition, one preparation bar, and the primary publication CTA. Notification, import, new event/edition, and destructive operations are grouped under `Actions`; deletions keep their typed confirmations. Event search and selection share one compact combobox: focusing it opens the authorized list, typing filters that same popup, and keyboard arrows plus Enter select an event. The control is capped at 28 rem on desktop instead of filling the header. `Gérer la visibilité` starts closed and contains the edition catalog switch plus one accessible three-state radio group per format. Its visible choices remain the short `Masqué`, `Privé`, and `Public` labels; each longer consequence appears contextually on hover or focus and is programmatically associated with its radio. The same short-label/contextual-help pattern applies to secondary field and action guidance throughout the dashboard. Errors, missing-data warnings, persistence state, and inherited values remain inline. `Masqué` writes `racebook_preview_is_visible = false` and `racebook_is_live = false`; the format remains editable in the authorized workspace and its navigation choice is grey and explicitly marked as hidden from the public. `Privé` keeps preview true and live false; `Public` requests both true through the existing membership-, readiness-, and entitlement-checked route. If the selected edition has only the free Visibility tier, choosing `Public` opens the publication-offer dialog immediately; a `403` reopens it only while the displayed tier is still Visibility. With an active Essentiel, Complet, or Signature entitlement, failures remain operational errors and never become an upsell. Incomplete formats and formats under a hidden edition cannot choose Public. The primary `Publier` CTA remains the bulk path for all private-selected complete formats. From Visibilité, it presents Essential 99 € HT, Complete 199 € HT and Signature 349 € HT. With any paid or complimentary tier already active, it publishes directly and the public serializers hide selected sections above that tier instead of forcing an upgrade. A trusted admin can still open the offer flow separately and grant a higher tier without Stripe. After checkout, publication waits for the trusted webhook-confirmed entitlement; URL parameters never grant access. Completion includes only effectively active modules and remains independent from catalog and RaceBook visibility. Completion tiles keep the module name, a compact textual empty/partial/complete status, and the content count; grey/orange/green surfaces are only a redundant cue. Detailed missing-field lists remain omitted, and the active tile remains emphasized by its selection ring.

On first arrival for each active event membership, the dashboard opens a six-step spotlight guide covering the overview, selectors, common-versus-format navigation, completion tiles, active editor, and visibility controls. It waits until the user has answered the cookie banner and temporarily suppresses the mobile-app prompt, so onboarding surfaces never stack. The editor step targets only its compact header, while the final step temporarily expands and highlights the real visibility controls; leaving that step restores the collapsed presentation. Completion or skip is stored on that organizer's `race_event_organizers` row, so co-organizers progress independently and the result follows the account across browsers. Trusted admins never receive the automatic guide but can replay it, as can organizers, through `Actions > Revoir le guide`. `Terminer` is the primary final action; the optional secondary action opens the existing section chooser. That chooser no longer auto-opens independently.

The Organizer access editor mirrors the mobile reading hierarchy without changing storage: `Lieux et itinéraire` groups start/finish autocomplete plus the optional general map, `À retenir` groups the priority note and road restrictions, and `Venir sur place` groups parking, shuttle operation, and shuttle schedule. Format-level section switches stay beside the group they control, and the runner-specific toggle remains separate. Mobile publishes format-specific runner information only when both `access.overrideEnabled` and `access.enabledSections.runnerInfo` are true; turning either control off preserves the JSONB values but removes them from the RaceBook. All values continue to use the existing event/format `organizer_details.access` JSONB object; this presentation change requires no migration and remains compatible with historical access payloads.

The common `Informations` form is ordered by task importance: `Informations principales` contains name and geocoded place, `Présence en ligne` contains the official and social URLs, `Dates de l’édition` contains the canonical range, `Contact d’urgence` receives a distinct warning-toned surface, and `Image de couverture` keeps its preview beside the upload control. This hierarchy changes no field ownership or save payload.
...
The completion shell does not repeat a local heading or helper sentence above the tabs. It separates the edition-wide `Commun à toutes les courses` scope from the `Formats de course` group on desktop and through `<optgroup>` labels on mobile. Every selected-edition format remains available to authorized organizers/admins: a masked choice is grey and explicitly labelled `Course masquée pour le public`, a private choice carries the discreet `RaceBook privé` label, and a public choice states `Course et RaceBook publics`. The responsive native selector includes the same wording. The active scope uses a brand border, tinted surface, ring, and selection marker so it remains distinct from content completion.

Equipment, bib pickup, and access are split by tab in the UI. On format tabs, the main module header owns the contextual `<module> - <format>` title and right-aligns its specific-value toggle. Disabled toggles render no format fields and resolve runner data from the event; enabled toggles reveal the full format editor without an additional enclosing panel. Bib pickup accepts several event-level pickup locations and can be overridden with `Retrait différent pour ce format`; each location owns its geocoded address and any number of structured date/start/end slots. The first address remains mirrored into the legacy single-location fields, and historical free-text schedules remain editable as compatibility information until replaced by structured slots. Equipment uses `mandatoryEquipment.overrideEnabled`, while access uses `access.overrideEnabled`; enabling either starts from the event value and then stores a complete format replacement. Historical access JSON with meaningful format values and no flag remains a legacy override, while an explicit `false` always inherits the event. The event equipment editor owns the active weather plan radio group (`normal`, `grand froid`, `grosse chaleur`), while each item keeps its own `cold` / `heat` toggles and required/recommended radios. Those per-item controls stay inline on the same flexible row as the label. Existing legacy format differences are promoted to checked overrides when the dashboard loads. The add-format tab can prefill a new format draft from event defaults or the previously active format. Event and format rows display read-only live/brouillon badges; organizers submit one event-level publication request instead of toggling liveness directly. Dirty state is scoped per event or race. Changing event, edition, format tab, or module updates the UI immediately and queues the previous scope's save in the background without a success toast or visible saving state; saves for the same scope are serialized, `beforeunload` remains armed while any scope is dirty, and failures remain visible. Saving the format-level Ravitos module writes both `races.organizer_details.schedule` through the race route and the station rows through the aid-station route.
...
The initial dashboard read uses `GET /api/organizer/bootstrap`. It preserves the four collections returned by `GET /api/organizer/claims` and adds the selected event detail after a single bearer-token verification. A requested `eventId` is accepted only when present in the caller's active membership set, or in the trusted admin's complete event selector. Without an explicit selection, the first selectable event is loaded; with no active membership, the event detail is `null`. Heavy format sidecars, GPX parsing, products, runner updates, follower information, and editable service/sponsor/branding collections are excluded from this bootstrap contract. GPX is requested only when Course or Ravitos needs it; ravito/relay/product sidecars and the catalog wait for Ravitos or Products; announcements wait for the notification dialog, while the same event-wide follower total is loaded independently by either the notification dialog or the authorized Statistics request. The bootstrap nevertheless includes lightweight persisted status projections for ravitos, SAS, podiums, services, active sponsors/clicks, and branding published/draft state, so tiles are correct before their lazy editors open.

Both the bootstrap and subsequent event-detail reload include `racebook_preview_is_visible` alongside `racebook_is_live` for every format. `Masqué` stores `is_live = false`, `racebook_preview_is_visible = false`, and `racebook_is_live = false`. `Privé` stores `is_live = false`, `racebook_preview_is_visible = true`, and `racebook_is_live = false`, so runners receive the course for plan creation while only active organizers receive the RaceBook preview action. `Public` atomically stores all three flags as true after the existing readiness and entitlement checks.

Module-scoped reads reuse a bounded route-local in-memory cache: the product catalog is fresh for 5 minutes, complete ravito/relay/station-product sidecars are isolated by race id for 2 minutes, and parsed GPX previews are isolated by race id plus `gpx_storage_path` for 10 minutes. LRU eviction caps sidecars and GPX at 20 races, with at most three GPX paths per race; expired entries are purged on writes. A GPX path replacement therefore misses without comparing payload contents. Successful mutations invalidate the affected race snapshot; changing the authenticated user clears every Organizer cache entry.
...
Only required tiles advance the dashboard percentage. Module tiles no longer expose `Complet`, `Obligatoire`, `Recommandé`, `Optionnel`, or `Modifier` badges: their title and content summary are primary, actionable missing fields remain visible, and grey/amber/green card treatments communicate empty/partial/complete state. A separate brand ring identifies the selected tile without replacing its completion color. Recommended and optional modules still retain their underlying completion semantics but neither raise nor lower the bars.
...
- bib pickup = the complete format value only when its override is enabled, otherwise the event value, with every structured pickup location and its dated start/end slots preserved; legacy single-location/free-text schedules remain fallbacks;
- access = the complete format value only when its override is enabled, otherwise the event value, with parking, shuttles, road restrictions, and map content filtered by the resolved enabled access sections;
...
The mobile Racebook view uses the same merge rules for live formats, but keeps organizer content read-only and compact. Its top identity card contains event/format identity, the event date range, the best published location, and runner information, while distance, D+, D-, and start-time metric pills are omitted from this synthesis. Its flexible metadata row uses calendar/location icons, dot separators, and compact `Solo` and/or `Relais` participation badges; mixed participation uses two separate badges rather than one combined label. When the format date differs from the event start date, it is emphasized in a bordered calendar row labeled `Jour de course :` / `Race day:`. Weather and last-minute messages remain dedicated compact alert cards immediately below it. The route-local tabs remain `Matériel`, `Dossard`, `Course`, and `Accès`, with a fifth `Services` tab only when event service details are populated; every tab target is at least 44 points high. Published official-site and social values render as accessible icon-only outlined actions beside the race identity. The emergency row keeps `Urgence - nom - téléphone` beside a localized call action and opens the platform phone application through a normalized `tel:` URL; French values normalize to `+33 X XX XX XX XX`, and the phone still counts as meaningful non-ravito content for availability. Equipment remains split into required, recommended, and weather-conditional groups, but mobile presents it as an accessible runner checklist with an accent-colored completed/total summary; checks are personal state persisted by account and exact format, while organizer equipment remains unchanged. Cold/heat markers remain descriptive icons and conditional rows are not styled as disabled. `Dossard` groups locations and same-day slots in one `Où et quand` card, exposes Maps through a separate 44-point icon action, then separates documents and rules; the legacy free-text schedule appears only when structured slots are absent. `Course` keeps its compact departure/finish summary for `Tracé` and `SAS`; `Ravitos` instead places the common start time inside the departure card and the schedule finish cutoff inside the arrival card, including when a source ravito at race distance becomes the finish. Segment summaries retain equal spacing above and below. Tracé keeps the stored GPX map and elevation profile, SAS rows expose ordered names, native start times and criteria directly, and Relais keeps derived legs only when published relay points produce them. `Accès` prioritizes road/access warnings, then start/finish locations and transport details; each optional block remains omitted when its format-level flag is disabled, and Maps actions meet the 44-point target. Structured Services group by category with compact directions/site/phone actions, while legacy service prose shares one light card. A native pull-to-refresh re-queries the complete published RaceBook snapshot, profile, and route so organizer changes can appear without restarting the app; a failed refresh keeps the last successful snapshot visible.
...
Organizer aid station edits preserve submitted station ids so `race_aid_station_products` links survive. Replacing ravitos, station-product links or relay points executes in one locked database transaction per race, so validation or insertion failure rolls the complete collection back. Creating an organizer product and attaching it to its station is also one transaction and cannot leave an orphan product. New or legacy stations default all service flags to enabled unless an organizer disables water, solid food, or assistance explicitly.

Aid station `organizer_details` stores cumulative D+/D-, cutoff time, drop-bag availability, and organizer note on the station row; legacy `stationType` and `altitudeM` values may still exist in persisted JSONB, but the current organizer dashboard no longer exposes editors for them. These fields must still be saved through the organizer aid-station route so existing station ids are kept. In the current organizer UI, ravitos use the same expandable card pattern as the runner planner: the compact card keeps distance, cumulative D+/D-, cutoff, water/solid/assistance/drop-bag toggles, and product actions visible first, while the expanded panel goes directly from the main info grid to the organizer note block. When an active-format GPX preview is available, editing a ravito km now recomputes cumulative D+ / D- automatically from the GPX trace and the corresponding form fields remain read-only. The same ravito tile also owns the fixed `Départ` and `Arrivée` timing cards for the format. In the mobile read-only RaceBook, a ravito card keeps its cumulative kilometer at right and services/cutoff below its name; the separate connector shows the segment distance, D+ and D-. Those segment D+/D- values are computed from cumulative station values, falling back to the first station's cumulative values when there is no previous published ravito.

On mobile, the Racebook presents ravitos on a continuous vertical chronology. Each card keeps the cumulative kilometer, available-service icons, and optional cutoff; the connector before it always exposes distance, D+ and D- from the previous ravito or from the start, with its summary indented to the right of the station card edge. Products and organizer notes remain in the single-open expandable content, while a station without either is a non-interactive summary card. This is presentation state only and does not alter source station ordering or organizer data. Its access and bib information rows also size the value from its content: short values let the divider run to the value, while long linked locations keep a bounded width and wrap cleanly.
...
For relay-capable formats, the `Ravito / relais` module separates its content into local `Ravitos` and `Relais` tabs. The ravito view is the default on initial mount and is restored after changing format or participation mode; it contains the start card, ordered ravito cards, and finish card. On medium and larger screens, the add action for the active local view is aligned with the module title and removed from the content flow so the tabs sit directly below the header; it stays in the normal flow on smaller screens. The relay view puts its compact derived legs in one horizontal row above the handover-point block; each point card exposes only its name, distance, cutoff, and compact delete cross. The optional persisted passage time and notes stay compatible with existing rows but are not edited in this interface. Solo formats do not show a relay tab. The tab choice is local presentation state and does not alter dirty tracking or save payloads.
...
Sponsors are optional and scoped to the selected canonical edition, so all its formats share the same presentation. Any active organizer may prepare the selected sponsor module as a private draft; Signature controls runner presentation, redirects, and aggregate impressions. The editor lazily loads the selected edition's list and supports up to ten rows. Alongside name and optional HTTP(S) website, each row stores a `principal` / `official` / `service` level, an optional short category, and an optional exact contextual placement in Ravitos, Matériel, Accès, or Services. Activation and loading/hero placements remain independent. Ordering sends the complete id/position list to one locked database transaction; partial lists, foreign ids and duplicate positions fail without a partial reorder. Dirty metadata keeps `beforeunload` protection armed until persistence, and edition changes remount the editor. Public click redirects and impression counts stop while the sponsor module is ineffective.

Logos are required raster uploads (PNG, JPEG, WebP, or AVIF, 5 MB maximum) stored under `race-images/organizer-sponsors/{editionId}/`. Organizer routes verify active parent-event membership and the Pro `sponsors.manage` capability for every read/write, then clean replaced or deleted objects. Edition/event deletion also collects sponsor logo paths before the database cascade and removes them afterward. Ordinary runners receive existing active placements only while the corresponding RaceBook passes its normal visibility gate; entitlement downgrade blocks organizer management without changing that public presentation rule.

The public presentation route exposes active placements, hierarchy/category metadata, and counted redirect URLs, never the target website field or counters. Redirects validate the sponsor/race edition pair, increment one aggregate click counter through an atomic RPC under a per-sponsor/network-hash rate limit, and continue to the sponsor site if counting fails. Viewable sponsor surfaces call the separate aggregate impression endpoint with a random view UUID; the endpoint deduplicates, rate-limits, and repeats publication/module/placement checks without storing a runner identity or individual history.

On mobile, pressing RaceBook starts the authorized sponsor lookup and loading-logo warmup before navigation. The loading interstitial remains bonus visibility only: it can show at most two partners for the existing 2.5-second minimum once logos are ready, but it is transient and not the main commercial placement. The stable themed partner block in the RaceBook hero is the principal sponsor surface; it uses a subtle unframed heading and compact tiered rows rather than oversized nested advertising cards. Matching contextual partners can then appear in Ravitos, Matériel, Accès, or Services without being inserted indiscriminately between content cards.

Active partners remain visible in the stable themed sponsor surface, grouped by hierarchy and contextual placement. The former auto-rotating compact carousel is not part of the current presentation.
...
The existing public sponsor payload transports only resolved published branding. With `RACEBOOK_EDITION_LOGO_ENABLED` enabled, a valid HTTPS logo is prefetched and rendered; disabling the flag forces it to `null` without deleting stored assets. The shared theme resolver owns defaults, contrast text, and derived light surfaces/borders for both colors. Primary styles identity and navigation controls; accent is used beyond the trace for checklist completion, structured relay/SAS/podium cards, decorative location/service actions, positive information and schedule highlights. Mobile waits for that edition payload before revealing content, preventing a slow request from leaving one format on defaults. Layout, typography, neutral card backgrounds, sponsor placements, native navigation, the runner illustration, and semantic danger/warning/info colors remain Pace Yourself-owned.

## Mobile Scope

No mobile organizer editor exists in v1. Ordinary runners see preview-selected private formats in Courses and may create plans from them, but can consume organizer details through the read-only `race/[id]/racebook` screen only when the format is live, `races.racebook_is_live = true`, and there is meaningful non-ravito organizer content. Active event organizers may open a private format's same runner-facing screen for preview. A masked format has no mobile catalog entry for any role and remains editable only from the authorized web workspace. Aid stations by themselves must not surface the Racebook entry point. The screen never grants edit or admin powers.
...
The Courses list keeps multi-format event cards compact by omitting the repeated format-choice helper sentence and decorative flag, making the main card surface open format selection, placing the compact format action on the same row as the format-count and distance pills, filling the left rail with the event/format image, and moving the independent animated heart into the trailing header position. Horizontal quick chips beside the advanced filter action filter favorites or the `< 30 km`, `30–60 km`, and `60+ km` distance bands in memory. The onboarding use of the shared card may retain the guidance. This is presentation-only and does not change organizer format visibility or grouping.
...
The current publication gate is edition-scoped Visibilité/Essential/Complete/Signature, documented in [Organizer Commercial Offers](organizer-commercial-offers.md). A paid or admin entitlement replaces new admin publication requests: organizers can publish complete formats immediately after the webhook or audited complimentary grant activates the edition. Complimentary actions are visible only to a trusted admin and do not create Stripe payment rows. Notifications and duplication require Complete; relay writes, official station products, sponsor management, branding and assisted import require Signature. Historical publication requests remain audit/compatibility data, and legacy RaceBook/Pro rights map to Complete/Signature.
...
- Do not fold the official edition logo into sponsor placements or sponsor click reporting; both asset families share a bucket but use distinct prefixes and contracts. Its runner rendering remains controlled through the enabled shared feature flag.
...
- Les documents sélectionnés par `Importer les informations` sont limités à 25 Mo par fichier et transférés directement depuis le navigateur vers le bucket privé temporaire `organizer-imports`, afin de ne pas traverser la limite de charge utile Vercel. Le transfert TUS est reprenable, découpé en blocs de 6 Mo, affiche une progression agrégée et peut être annulé; un échec ou une annulation nettoie les objets déjà terminés. Un PDF seul peut lancer la découverte sans URL principale. La session conserve uniquement les chemins nécessaires aux deux passes; apply, annulation ou nettoyage d'expiration supprime les objets. Les PDF texte produisent des claims paginés à confirmer; les images et PDF sans texte restent signalés `ocr-pending` tant qu'aucun OCR n'est disponible.
...
- Format-specific access is also an explicit opt-in override. An explicit `false` inherits event access even if stale format fields remain; only historical JSON with an absent flag and meaningful access data may retain the former merged behavior.

- Keep automatic generic discovery same-origin, hint-filtered, and capped. Explicit headings on the primary page may establish formats, and KMS-style `Event:Format 16KM 385m D+` labels are normalized without merging equal type names at different distances. Repeated registration calls-to-action must not override a high-confidence multi-format classification. Automatically linked PDFs require an explicit PDF signal and remain bounded evidence sources. Explicitly supplied additional official URLs may be cross-origin because the admin selected them, but they must still be classified and evidence-grounded before contributing claims.
...
- The normal Organizer format editor participates in the same lifecycle. Saving the last missing date, location, positive distance, or source changes the imported row to `complete` while keeping `is_live = false` and `racebook_is_live = false`. Clearing a required catalog value makes the row a hidden draft again. The dashboard exposes the draft state and the remaining required fields after reload.
- Before the first pricing dialog opens, the dashboard checks the selected edition for an effective format name, date, location, positive distance, and official source. Missing data redirects attention to the format editor instead of deferring the error until an offer is chosen.
...
- Structured collection autosaves must compare the saved revision with the current revision before accepting a normalized response; a pending edit must trigger the next serialized save.
...
- Opening visibility controls for the last guide step is temporary presentation state. It must neither persist a visibility choice nor leave the panel forced open after the guide moves away or closes.
...
- Keep direct organizer assignment and e-mail autocomplete admin-only and server-side. The browser may receive only the bounded matches for its current query, never the Supabase service credential or the complete Auth user list. A missing e-mail must remain manually enterable and be returned as a confirmable not-found result; account invitation/creation happens only after the admin confirms. Keep `/reset-password` in Supabase's allowed redirect URLs and pass it through the invite endpoint's `redirect_to` query parameter. Supply the canonical name as `data.event_name` for template display only; authorization still comes exclusively from active membership. Existing-account notices must be sent only after the membership succeeds, must link to the selected event, and must not be duplicated for a newly invited account. Assignment must not implicitly publish or unpublish the event or its Racebook formats.
...
- The runner RaceBook replaces the global app bar while focused with effective primary sections plus an explicit Courses exit. Its hero return action, contextual Courses action, and Android hardware back all replace the active route with Courses instead of reopening a previously viewed RaceBook. Its branded hero uses the return row for its title and progressively becomes the sticky top navigation; a valid emergency contact appears with number/action when expanded and as a phone icon before persistent social actions when compact. Participation remains metadata, official partners use compact tiered rows, and full-width departure/arrival cards hold their schedule times at the content edge while the unmarked ravito rail leaves the first card and enters the last. These are read-only presentation rules and do not grant, publish, or mutate organizer modules.
