...
- Les documents sélectionnés par `Importer les informations` sont envoyés en multipart, avec une limite de 25 Mo par fichier, et un PDF seul peut lancer la revue sans URL principale. Les PDF contenant du texte sont extraits temporairement côté serveur. Les lignes reconnues produisent des observations à confirmer pour les métriques, départs, dossards, barrières, ravitos, matériel, secours et suivi live ; elles sont rattachées à un format connu seulement par nom ou distance compatible, puis comparées aux données actuelles du format. Les résultats indiquent un champ à remplir, une valeur identique ou une différence à valider avant écrasement ; les images et PDF sans texte restent en `ocr-pending`, et aucune observation n'est écrite automatiquement dans une course.
