...
The format identity editor now uses a desktop two-column layout with a flatter hierarchy: a compact information column on the left and a dedicated file side rail on the right. That right rail keeps only the GPX upload first and the image upload second, while the interactive route map and the denser elevation profile move into a larger full-width block underneath so the course preview becomes the main visual focus whenever GPX coordinates are available.
