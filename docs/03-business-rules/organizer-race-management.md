...
On mobile, the Racebook presents ravitos as a single-open accordion. A collapsed summary keeps the station name, distance, available-service icons, segment D+/D-, and optional cutoff visible; opening it reveals the full right-hand metric column, labeled service controls, organizer products, and notes. Water, solid food, assistance, and drop-bag controls remain icon-only buttons in the expanded content; tapping one toggles a single inline information bubble with the service label. This is presentation state only and does not alter source station ordering or organizer data. Its access and bib information rows also size the value from its content: short values let the divider run to the value, while long linked locations keep a bounded width and wrap cleanly.
...
Sponsors are optional and scoped to the selected canonical edition, so all its formats share the same presentation. Any active organizer may prepare the selected sponsor module as a private draft; Signature controls runner presentation and click redirects. The editor lazily loads the selected edition's list and supports up to ten rows. Name and optional HTTP(S) website metadata save on blur or explicit save; activation and loading/banner placements save immediately. Ordering sends the complete id/position list to one locked database transaction; partial lists, foreign ids and duplicate positions fail without a partial reorder. Dirty metadata keeps `beforeunload` protection armed until persistence, and edition changes remount the editor. Public click redirects return `410` while the sponsor module is ineffective, so hidden sponsors cannot continue recording clicks.
...
On mobile, pressing RaceBook starts the authorized sponsor lookup and loading-logo warmup before navigation. A short-lived request cache is scoped by account id and race id, so the destination reuses the same in-flight result without leaking organizer-preview data across sessions; direct links still perform the full lookup themselves. Visible progress waits at its initial position until logo preparation settles. Loading placements then use a dedicated RaceBook composition: a localized preparation title and unframed runner progress trail sit directly above one unified sponsor panel. That panel reserves up to two vertically stacked logo slots, separated by a subtle divider, and occupies roughly one third of the viewport. An empty or failed lookup removes the panel and adds no sponsor delay. Feedback and the bottom tab bar stay hidden only during this loading state, while the native back/title header remains available. This presentation does not change placement limits, edition scope, click counting, or the existing 2.5-second minimum gate once sponsor logos are ready. That minimum is intentionally independent from backend speed: a warm CDN or in-memory response must not make configured loading sponsors flash or disappear.
...
The existing public sponsor payload transports only resolved published branding. While `RACEBOOK_EDITION_LOGO_ENABLED` is false, its logo is forced to `null`, is not prefetched, and is not rendered. The shared theme resolver owns defaults, contrast text, and derived light surfaces/borders for both colors. Primary styles controls plus structured relay, SAS, and podium cards; accent is used beyond the trace for related graphic cards, positive information and decorative schedule highlights. Mobile waits for that edition payload before revealing content, preventing a slow request from leaving one format on defaults. Layout, typography, neutral card backgrounds, sponsor placements, native navigation, the runner illustration, and semantic danger/warning/info colors remain Pace Yourself-owned.
