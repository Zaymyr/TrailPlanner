...
The generic fallback keeps event facts and common logistics on the general URL, but it can also extract strongly structured format blocks exposed as accessible tabs on that same page. When the organizer supplies no explicit format URL, it follows at most six same-origin links whose labels or paths identify likely race sources such as regulations, courses, formats, schedules, or roadbooks. Explicit format URLs disable discovery and are authoritative: an embedded route on the general page must not filter, replace, or collapse candidates extracted from those pages. Event-level JSON-LD repeated on a format page is ignored as a format candidate when the same page exposes a more precise visible course identity. When an explicit page embeds one or more complete routes, each route acts only as an anchor for that page: compatible nearby metrics are consolidated into it, while incompatible relay legs and incidental distances are not promoted to extra formats. All secondary pages are fetched concurrently, with each fetch capped at eight seconds and oversized HTML truncated before parsing. The extractor scores dates from their surrounding copy so a race date outranks registration deadlines, recognizes format sections under headings from `h1` through `h6`, and understands named regulation prose such as `« Fleurinoise » d'une longueur de 18 km`. It rejects distance mentions that belong only to ravitos, barriers, age groups, prices, results, or analysis blocks; anonymous `15 km` detections remain visible beside named candidates until format confirmation resolves them. Accessible tab panels may expose consecutive distance, D+, and D- metrics plus a format-specific GPX link; parsed GPX measurements still override those HTML values after an unambiguous format association.
...
- When explicit format URLs are supplied, do not use a featured/embedded route from the general page as a global candidate filter. Repeated event JSON-LD must not erase the distinct visible identity of each explicit page. If the general URL is also present in the explicit list because it is one format page, fetch it once and include its visible format alongside the sibling pages.
...
- Keep automatic generic exploration same-origin, hint-filtered, size/time-limited, and bounded. Explicit format URLs remain authoritative additions, while missing formats stay addable manually in the first review.
