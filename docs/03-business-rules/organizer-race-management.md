...
Revoking access still sets `revoked_at` on the membership and blocks future organizer writes without removing the course from the catalog. Yearly editions and formats are catalog-visible, but each new Racebook starts hidden. RaceBook publication is edition-entitlement gated: the server filters every module outside the active tier without deleting its draft. A trusted admin may grant a paid offer without payment through the audited `source=admin` entitlement boundary; free draft editing never creates such a grant.
