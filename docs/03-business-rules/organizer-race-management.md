...
- access = format value when filled, otherwise event value, filtered by enabled access sections, except that a populated parking value remains visible even when the format-level parking flag is false;
