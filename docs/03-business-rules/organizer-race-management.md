...
On mobile, loading placements use a dedicated RaceBook composition: a localized preparation title and unframed runner progress trail sit directly above one unified sponsor panel. That panel reserves up to two vertically stacked logo slots, separated by a subtle divider, and occupies roughly one third of the viewport. It fills as soon as the lightweight sponsor lookup returns; an empty or failed lookup removes the panel and adds no sponsor delay. Feedback and the bottom tab bar stay hidden only during this loading state, while the native back/title header remains available. This presentation does not change placement limits, edition scope, click counting, or the existing 2.5-second minimum gate once sponsor logos are ready.
