...
The format identity editor now uses a desktop two-column layout with a flatter hierarchy: a compact information column on the left and a dedicated GPX side rail on the right. The left column keeps the key race fields tighter on one grid and moves image upload into a lighter secondary row. The right rail shows the GPX upload state first, then a larger interactive route map and a denser elevation profile underneath whenever preview coordinates are available.
