...
The current publication gate is edition-scoped Visibilité/RaceBook/Pro, documented in [Organizer Commercial Offers](organizer-commercial-offers.md). A paid or admin entitlement replaces new admin publication requests: organizers can publish complete formats immediately after the webhook or audited complimentary grant activates the edition. The two complimentary actions are visible only to a trusted admin and grant RaceBook or Pro without creating a Stripe payment row. RaceBook keeps `Notifier les coureurs` visible as a 100 € HT Pro upgrade; server routes still refuse notifications, duplication, relay writes, official station products, sponsor management, and assisted import without Pro. Historical publication requests remain audit/compatibility data, and approving one grants Pro.
