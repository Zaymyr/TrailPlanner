...
- Consolidate near-distance detections with the bounded 0.5 km / 1% tolerance, raised to 0.8 km / 1.5% only when one candidate is named and the other is distance-only. Retain the earliest heading-level name and preserve a conflict warning when non-GPX metrics disagree. Do not merge meaningfully different distances merely because their labels overlap.
