...
- expand the compact edition/format summary from its status line; it is closed by default without changing any publication state, completion score, unsaved edit, or selected scope, while the offer and primary publication/notification actions remain visible;
- permanently delete the selected event from the explicit destructive action inside the expanded summary, but only after typing the exact word `Supprimer` in the confirmation dialog; the server restricts this action to the active owner membership (or a trusted admin), removes event-owned formats and Storage assets, and leaves saved runner plans detached from their deleted source formats;
...
The dashboard is organized as a compact top synthesis plus one tabbed completion surface. The event selector has an explicit label; when more than eight events are available, a name filter narrows its options while retaining the selected event. The edition/format summary is closed by default, but its compact line still exposes visibility, format count and completion. The offer, notification and publication actions remain outside the collapsed content. Expanding it reveals the year selector, edition and per-format visibility/progress rows, plus explicit 44 px-high edition/event deletion actions separated from selectors; the existing typed confirmations remain mandatory. `Créer une nouvelle édition` opens the date-range dialog; creating an empty edition is free, while duplication requires Complete or Signature. On mobile, a labelled select replaces the horizontally scrolling event/format tabs; desktop keeps the tabs and gives the add-format action an accessible name. Dirty work exposes a fixed save bar until the save completes. Every format row has one entitlement-free `Format visible dans ma démo` switch, even before publication. Turning it off removes the whole format from the organizer's mobile demo, including the event's format count and selector, and also unpublishes it if necessary; the format remains in the web management dashboard so it can be restored. Turning it on never exposes it to runners. The separate `Publier les RaceBooks` action atomically publishes only preview-selected complete formats after the offer check. From Visibilité, it presents Essential 99 € HT, Complete 199 € HT and Signature 349 € HT; from a paid tier it can publish directly or present valid upgrades for private advanced sections. A trusted admin can grant any paid tier without Stripe and the displayed value comes from the shared tier catalog. After a publication checkout, the dashboard polls the trusted edition entitlement and performs publication only after the webhook confirms it; URL parameters never grant access. A hidden edition guarantees its public RaceBook flags stay false. Completion includes only effectively active modules and remains independent from catalog and RaceBook visibility.
...
- Keep new-edition dates and the optional duplication control inside the shared dialog; the expanded compact summary contains the edition selector, visibility switch, creation button, and explicit deletion action, with destructive controls separated from selectors.
