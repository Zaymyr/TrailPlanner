...
- The admin purchase dialog identifies the currently active pack and whether it is complimentary. Bank-transfer failures must remain visible inside that dialog; the page-level error alone is obscured while the modal is open. A same-tier complimentary grant may be converted to a real paid purchase, while an already paid same-tier purchase remains a duplicate.
