...
A trusted admin can choose the effective publication origin shown for an edition: operational `admin`, `complimentary` (Offert), ledger-backed `stripe`, or ledger-backed `manual_payment` (virement). Admin and Offert grants may be created or changed directly. Stripe can only be selected when a matching valid payment path exists. Selecting a new virement is itself the payment-ledger write: the server derives the canonical direct price (99/199/349 € HT), records zero VAT because the declared issuer uses article 293 B CGI, validates the customer billing name and address, accepts a missing SIREN for an organization that has none while validating any supplied SIREN as nine digits, records the selected calendar date at the start of that UTC day, and grants the purchased tier atomically. It then allocates a chronological `PY-YYYY-NNNNNN` invoice number, freezes the legal snapshot, renders the PDF with `pdf-lib`, and stores it privately. Using midnight prevents a same-day payment from being rejected as future before noon UTC. Future dates, incomplete billing identity, duplicate paid tiers, paid-ledger downgrades, and invented paid origins are rejected. An existing `admin`, `complimentary`, or `legacy_admin` grant can be replaced by a real bank-transfer purchase, including a lower paid tier chosen deliberately by the admin.

The admin dialog can preview the exact server-rendered PDF without allocating a number or writing data. A paid historical bank transfer that has neither an uploaded nor generated invoice exposes `Générer une facture`: the admin supplies the customer identity, previews from the ledger's stored tier/date/amount, then issues on that same payment row without recreating the transfer or changing the entitlement. The organizer bootstrap exposes the effective pack, payment channel, date, amounts, invoice number, and availability. `Actions > Factures` lists paid/refunded/disputed history for every edition of the selected event. Every active event member may request a download; generated and legacy manual PDFs use a short private Storage URL whose relative Supabase path is resolved below `/storage/v1`, while old Stripe rows resolve their Invoice from Checkout on first download. DTOs never expose provider ids or private object paths.
...
- Client identity means legal name and billing address. SIREN is optional; if supplied it must contain nine digits, and when absent the shared preview/final renderer omits the SIREN line instead of printing an empty value.
...
- Historical invoice issuance is available only for a paid EUR bank transfer with no existing invoice, zero stored tax, and `amount_total = amount_subtotal`. Do not silently substitute today's catalog price or mutate the old payment to make it eligible.
- Preserve Supabase's Storage API base when turning `/object/sign/...` into an absolute invoice URL; resolving it against the bare project origin creates an invalid download despite successful membership authorization.
...
- Raising the complete access-tab assignment card above the following blurred card keeps its autocomplete listboxes visible across sibling stacking contexts; it is presentation-only and has no effect on pack selection, entitlements, payments, or invoices.
- The searchable e-mail/event fields in the separate `Accès organisateurs` tab grant membership only; their bounded Auth suggestions and local event filtering never create or change an offer, payment, invoice, or edition entitlement.
