...
- `.github/workflows/db-migrate.yml` runs serially after a migration reaches `main`. It uses `supabase/setup-cli@v3` and `actions/upload-artifact@v7`, calls the same static verification, compares remote migration versions with files in the repository, uploads the CLI listing as a diagnostic, previews the pending push with `--dry-run`, then deploys. A remote-only version blocks the job. CI must never repair migration history automatically: `migration repair` changes the tracking table without proving that the schema matches the repository.
...
`supabase/migrations/20260923064926_backfill_public_race_event_website_urls.sql` is an idempotent data-only catalog repair. It fills the 13 live public events whose canonical `race_events.website_url` was empty during the 23 September 2026 audit, using existing verified organizer values or verified official event domains. It also mirrors the selected URL into an empty `organizer_details.officialWebsiteUrl` for dashboard consistency. A non-empty current event or organizer value always wins; the migration changes no schema, visibility flag, ownership, grant, function, trigger, or RLS policy.
...
`supabase/migrations/20260911091935_fix_bulk_organizer_racebook_publication.sql` repairs the edition publication RPC after the three-state alignment. Selected complete public-source formats now move atomically from private to public by setting course, preview, and RaceBook visibility true; the function no longer filters out the private rows it is supposed to publish. It remains `SECURITY INVOKER` and executable only by `service_role`.

`supabase/migrations/20260911114106_expose_private_formats_in_visible_catalog.sql` makes preview-selected private formats readable in the runner course catalog whenever their parent event and optional edition are visible. Its private fixed-search-path helper returns only that parent-visibility boolean and avoids exposing edition rows through the Data API. Masked formats remain excluded, and `racebook_is_live` continues to gate the runner RaceBook action independently.
...
`supabase/migrations/20260829204018_add_racebook_edition_sponsors.sql` adds service-only `race_event_edition_sponsors`, ordered loading/banner placements, aggregate click counts, and an atomic race/edition-validated redirect increment RPC. RLS and explicit privilege revokes keep clients behind server routes. A transaction advisory lock plus trigger enforces ten sponsors per edition and two active loading sponsors even under concurrent writes. `supabase/tests/racebook_sponsors_checks.sql` verifies RLS, privileges, both limits, and the atomic click/impression increments inside a rollback transaction. Its impression fixture selects an already-published, preview-visible RaceBook instead of trying to bypass catalog-completeness or edition-visibility triggers with direct updates.
