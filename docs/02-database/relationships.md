...
- Organizer update history is event-child data; deleting an event cascades favorites, updates, and their read receipts. Deleting only a format keeps the update as event history and sets its optional `race_id` to null.
