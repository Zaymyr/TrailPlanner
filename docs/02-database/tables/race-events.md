...
- Generic discovery may use a newer regulation to reject old-edition candidates and may consolidate detections only from compatible normalized identity evidence. Explicit format URLs remain distinct from any featured route on the general page; ambiguous event JSON-LD cannot collapse their page-specific identities. Anonymous same-distance detections stay separate for admin confirmation. Missing values such as D+ remain explicit and do not invalidate confirmed format existence.
...
- Treat organizer-supplied format URLs as authoritative candidates. A general-page embedded route may enrich the event context, but it must not filter those explicit pages down to one format. When the general URL is repeated in the explicit list because it is a format page, parse that already-fetched page once as an explicit candidate.
