...
- The separate `Accès organisateurs` admin sub-tab also reads the complete event list only after the same trusted admin check. Its single event combobox filters that authorized list by name, location, or date and stores the selected event id without another public lookup. Submitting an existing e-mail creates/reactivates membership, then sends a transactional notice containing the canonical event name and a `/sign-in` link with a validated `/organizer?eventId=...` return target. If the account is absent, only explicit confirmation creates membership and sends an invitation with the canonical event name in `data.event_name` and the password-creation page in the `redirect_to` query parameter; completion redirects the authenticated user to `/organizer`. Both paths leave every `race_events` field, including `is_live`, unchanged.
...
- Runner favorites are event-scoped and are used by the mobile catalog to pin the whole event card above normal ordering when the catalog is loaded or refreshed.
- The mobile Courses summary card may omit its decorative flag, fill a full-height left rail with the event/format image, place the independent heart in the trailing header action, open format selection from the remaining card surface, and render its compact format action on the same row as the format and distance pills. Quick favorites and distance filters operate on the already-loaded rows. These presentation choices change neither the event/format relationship nor its visibility query.
...
- The post-load sponsor surface is also presentation-only: its unframed heading and compact simultaneous tiered rows show edition-scoped sponsors without changing event grouping, queries, or visibility.
