...
| `invoice_number`, `invoice_sequence`, `invoice_issued_at` | mixed | unique number; positive sequence | Chronological `PY-YYYY-NNNNNN` identity and issue time for a generated invoice. |
| `invoice_source` | `text` | `generated|uploaded` when an invoice exists | Distinguishes automatic invoices from legacy/manual PDFs. |
| `invoice_legal_snapshot` | `jsonb` | object for generated invoices | Immutable seller, customer, service, payment and amount facts rendered in the PDF. |
...
- A partial unique index on `invoice_number` prevents duplicate issued identifiers. Yearly allocation is serialized with an advisory transaction lock.
...
- The protected admin route accepts only Essentiel, Complet, or Signature, a non-future payment date, and validated client billing identity. Legal name and billing address are mandatory; SIREN is nullable for an organization that has none and is normalized to nine digits when supplied. The route derives 99/199/349 € HT, records zero VAT under article 293 B CGI, uses midnight UTC for the settlement date, and calls `record_admin_organizer_bank_transfer_invoice`; the service-role-only wrapper records the paid ledger row, recalculates the entitlement, and allocates the invoice number plus immutable legal snapshot in one transaction.
- The admin preview and final PDF use the same customer schema: an empty SIREN becomes `null`, while any non-empty value must contain exactly nine digits. The PDF does not render a blank SIREN line.
...
- The manual upload route rejects generated invoices. Their attachment may only be repaired by regenerating the PDF from the stored immutable snapshot and original issue timestamp.
