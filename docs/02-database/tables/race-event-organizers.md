...
- Supabase Auth e-mail lookup, bounded autocomplete, and invitation for direct assignment must remain in the admin-only server route with the service credential. Return only the current query's small matching subset, never the complete Auth Admin user list or service credential, and never invite an absent address before the admin confirms the not-found dialog. The deployed `/reset-password` URL must remain allowlisted in Supabase so the invite redirect is honored.
