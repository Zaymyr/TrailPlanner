...
- Admin assignment: searchable e-mail and event comboboxes help an admin choose an existing account and event. E-mail autocomplete starts at two characters and receives at most eight matching Auth identities from the protected route; final assignment still uses an exact case-insensitive e-mail match. If it is absent, an explicit UI confirmation lets the server create it through a Supabase invitation whose `redirect_to` query parameter targets `/reset-password` and whose `data.event_name` contains the canonical race label for the e-mail template. The membership exists before the recipient chooses a password, and the completed invite session opens `/organizer`. When the Auth account already exists, the server sends a separate transactional notice after the membership is created or reactivated; its CTA targets `/sign-in` and carries a validated return to `/organizer?eventId=<event-id>`. New delegated memberships use `role = 'organizer'` and `claim_id = null`.
...
- Active membership authorizes the sanitized invoice history, generated invoice number, and invoice downloads for every edition of the parent event. It does not expose Stripe identifiers, legal billing snapshots, or private Storage paths, and revocation blocks both list and download routes immediately.
...
- Supabase Auth e-mail lookup, bounded autocomplete, invitation, and existing-account notification for direct assignment must remain in the admin-only server route. Return only the current query's small matching subset, never the complete Auth Admin user list or service credentials, and never invite an absent address before the admin confirms the not-found dialog. The deployed `/reset-password` URL must remain allowlisted and must be passed as GoTrue's `redirect_to` query parameter. Invitation metadata such as `event_name` and notification copy are untrusted display context, never authorization sources. Send the existing-account notice only after persistence; delivery failure must not roll back an already active membership.
