...
- The shared organizer-details parser also handles race equipment/access override compatibility, event website/social URL validation, and event emergency-phone display normalization. Those behaviors are independent from station `organizer_details` and must not alter ravito rows.
