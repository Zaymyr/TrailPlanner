...
- Organizer station details: optional JSONB for station type, cumulative D+/D-, altitude, cutoff time, drop bag, and organizer note. The shared dashboard-detail parser also contains event/race schemas, including geocoded event/access fields and the event-level multi-location bib-pickup schedule, but the station JSON shape remains scoped to station metadata. Format-level start and finish times are not stored as fake aid stations; they stay in `races.organizer_details.schedule` and are only rendered alongside ravitos in the organizer UI.
...
- The shared organizer-details parser also handles race equipment/access override compatibility, event website/social URL validation and HTTPS prefix normalization for valid domain links, and event emergency-phone display normalization. Those behaviors are independent from station `organizer_details` and must not alter ravito rows.
