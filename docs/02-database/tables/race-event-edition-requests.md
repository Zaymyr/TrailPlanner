...
- Direct admin assignment, including a newly invited Auth account confirmed through the admin access dialog and completed through `/reset-password`, creates or reactivates only `race_event_organizers`; the invite carries the canonical event name as display metadata and the password page as GoTrue's `redirect_to` query parameter, but it does not create, reactivate, or review a legacy edition request.
