...
- Direct admin assignment creates or reactivates only `race_event_organizers`; it never creates, reactivates, or reviews a legacy edition request. A newly invited Auth account completes `/reset-password`, while an existing account receives a post-persistence transactional notice with the canonical event name and a sign-in link returning to the selected Organizer event.
- Searching an e-mail or event in the direct-assignment comboboxes is read-only and likewise never creates, reactivates, or reviews an edition request. Elevating their parent assignment card above the following access card only fixes listbox painting across the shared blurred-card stacking contexts.
...
- Its direct-virement fields derive canonical pack HT, zero VAT under article 293 B CGI, and a generated invoice from the required client name/address plus an optional validated SIREN; this payment behavior remains independent from edition-request review history.
- The shared invoice renderer omits the customer SIREN line when that optional value is absent; this remains unrelated to the retired request state.
- Generating a missing invoice for an older paid virement updates only that payment's invoice metadata and likewise never creates or reactivates an edition request.
