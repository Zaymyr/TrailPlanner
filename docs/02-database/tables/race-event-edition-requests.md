...
- Per-format private-demo switches stay independent from this retired workflow and other dirty scopes. They require organizer/admin access but no offer, and disabling one also clears its own `racebook_is_live`. The separate edition publication action requires the paid or complimentary entitlement and creates no edition-request row.
