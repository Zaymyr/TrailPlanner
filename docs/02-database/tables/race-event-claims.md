...
- Inside a relay-capable format's `Ravito / relais` editor, the local `Ravitos` / `Relais` views remain presentation-only, including the compact derived-leg row and reduced handover fields. They do not change the membership boundary or grant a separate mutation path; the existing organizer routes continue to authorize every save.
