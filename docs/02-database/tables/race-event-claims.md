...
- The grouped access editor (`Lieux et itinéraire`, `À retenir`, `Venir sur place`) is presentation-only and continues to save through the same membership-checked event/race detail routes; it does not add claim permissions or a new database structure.
...
- Keep direct admin delegation visually and structurally distinct from legacy claims in the `Accès organisateurs` sub-tab. Its e-mail and event comboboxes are presentation/search controls only; bounded e-mail suggestions must come from the protected admin route and must not create a claim. Assignment grants membership immediately but must not add a fabricated claim audit row. An existing Auth account receives a post-persistence transactional notice with the selected event name and a sign-in link returning to that event's dashboard. A missing e-mail requires a cancel/create confirmation before the server sends an invitation with the same event name as display metadata and `/reset-password` as GoTrue's `redirect_to` query parameter for the eventual organizer dashboard.
