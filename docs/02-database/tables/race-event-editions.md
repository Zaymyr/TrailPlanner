...
- Organizer bootstrap/event reads may aggregate narrow service, active-sponsor/click, branding publication, and effective Analytics-access projections on each edition for initial tile status; full editor collections and PostHog data remain behind their dedicated routes. Both read paths must resolve the same tier plus complimentary grant because the dashboard replaces one event snapshot with the other.
