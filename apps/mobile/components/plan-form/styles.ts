import { StyleSheet } from 'react-native';
import { Colors } from '../../constants/colors';

export const inputStyles = StyleSheet.create({
  input: {
    backgroundColor: Colors.surface,
    color: Colors.textPrimary,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.border,
    paddingHorizontal: 14,
    paddingVertical: 12,
    fontSize: 16,
    flex: 1,
  },
});

export const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: Colors.background },
  content: { padding: 20, paddingBottom: 48 },
  planOverviewHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
    marginBottom: 20,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    paddingHorizontal: 10,
    paddingVertical: 9,
  },
  planOverviewCopy: {
    flex: 1,
    gap: 3,
  },
  planOverviewTitle: {
    color: Colors.textPrimary,
    fontSize: 13,
    fontWeight: '800',
    lineHeight: 17,
  },
  planOverviewMeta: {
    color: Colors.textSecondary,
  },
  planOverviewSettingsIcon: {
    width: 34,
    height: 34,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.brandBorder,
    backgroundColor: Colors.brandSurface,
    alignItems: 'center',
    justifyContent: 'center',
  },
  planSummaryCompact: {
    marginBottom: 10,
  },
  planSummaryMetricsRow: {
    flexDirection: 'row',
    gap: 8,
  },
  planSummaryMetric: {
    flex: 1,
    minHeight: 76,
    justifyContent: 'center',
    gap: 6,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    paddingHorizontal: 12,
    paddingVertical: 10,
  },
  planSummaryMetricAccent: {
    borderColor: Colors.brandBorder,
    backgroundColor: Colors.brandSurface,
  },
  planSummaryMetricValue: {
    flexShrink: 1,
  },
  planSummaryValueRow: {
    flexDirection: 'row',
    alignItems: 'baseline',
    gap: 5,
  },
  planSummaryUnit: {
    color: Colors.brandPrimary,
    fontSize: 11,
    fontWeight: '800',
  },
  planSummaryMetricLabelAccent: {
    color: Colors.textSecondary,
    fontSize: 11,
    fontWeight: '800',
  },
  planSummarySecondaryMeta: {
    color: Colors.textMuted,
    fontSize: 12,
    fontWeight: '700',
    marginTop: 8,
  },
  sectionAccordionHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    minHeight: 44,
    marginHorizontal: -6,
    marginBottom: 8,
    paddingHorizontal: 6,
    paddingVertical: 8,
  },
  sectionAccordionHeaderSpaced: { marginTop: 16 },
  sectionAccordionHeaderCollapsed: {
    marginBottom: 4,
    paddingVertical: 6,
  },
  sectionAccordionHeaderContent: { flex: 1, marginRight: 10 },
  sectionAccordionTitleRow: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    alignItems: 'center',
    gap: 6,
  },
  sectionAccordionTitle: { marginBottom: 0 },
  sectionSummaryRow: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 6,
    marginBottom: 4,
  },
  sectionSummaryRowInline: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    alignItems: 'center',
    gap: 6,
    flexShrink: 1,
  },
  sectionSummaryChip: {
    backgroundColor: Colors.surfaceSecondary,
    color: Colors.textSecondary,
    borderRadius: 999,
    borderWidth: 1,
    borderColor: Colors.border,
    paddingHorizontal: 10,
    paddingVertical: 4,
    fontSize: 12,
    fontWeight: '600',
  },
  sectionSummaryChipInline: {
    backgroundColor: Colors.surfaceSecondary,
    color: Colors.textSecondary,
    borderRadius: 999,
    borderWidth: 1,
    borderColor: Colors.border,
    paddingHorizontal: 8,
    paddingVertical: 3,
    fontSize: 11,
    fontWeight: '600',
  },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 },
  sectionTitle: { fontSize: 16, fontWeight: '700', color: Colors.brandPrimary, marginBottom: 12 },
  label: { fontSize: 13, color: Colors.textSecondary, marginBottom: 6, marginTop: 4 },
  textInput: {
    backgroundColor: Colors.surface,
    color: Colors.textPrimary,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.border,
    paddingHorizontal: 14,
    paddingVertical: 12,
    fontSize: 16,
    marginBottom: 4,
  },
  stationInput: {
    backgroundColor: Colors.surfaceSecondary,
    color: Colors.textPrimary,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.border,
    paddingHorizontal: 14,
    paddingVertical: 12,
    fontSize: 16,
    marginBottom: 4,
  },
  row: { flexDirection: 'row', marginBottom: 4 },
  rowItem: { flex: 1 },
  toggleRow: {
    flexDirection: 'row',
    backgroundColor: Colors.surfaceSecondary,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.border,
    padding: 4,
    marginBottom: 12,
  },
  toggleBtn: { flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: 8 },
  toggleBtnActive: { backgroundColor: Colors.surface },
  toggleBtnText: { color: Colors.textMuted, fontSize: 14, fontWeight: '600' },
  toggleBtnTextActive: { color: Colors.textPrimary, fontWeight: '600' },
  fatigueSliderWrap: {
    marginTop: 2,
    marginBottom: 8,
    paddingVertical: 8,
  },
  fatigueSliderTrack: {
    position: 'relative',
    height: 10,
    borderRadius: 999,
    backgroundColor: Colors.surfaceSecondary,
    borderWidth: 1,
    borderColor: Colors.border,
    overflow: 'visible',
  },
  fatigueSliderTouchArea: {
    position: 'absolute',
    top: -12,
    left: 0,
    right: 0,
    bottom: -12,
    zIndex: 2,
  },
  fatigueSliderFill: {
    height: '100%',
    borderRadius: 999,
    backgroundColor: Colors.brandPrimary,
  },
  fatigueSliderThumb: {
    position: 'absolute',
    top: -8,
    width: 24,
    height: 24,
    borderRadius: 999,
    backgroundColor: Colors.surface,
    borderWidth: 2,
    borderColor: Colors.brandPrimary,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.12,
    shadowRadius: 4,
    elevation: 2,
  },
  fatigueSliderLabels: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    gap: 8,
    marginBottom: 10,
  },
  fatigueSliderLabel: {
    flex: 1,
    color: Colors.textMuted,
    fontSize: 12,
    fontWeight: '600',
    textAlign: 'center',
  },
  fatigueSliderLabelActive: {
    color: Colors.brandPrimary,
  },
  sectionTimingResetBox: {
    gap: 8,
    marginBottom: 10,
    padding: 12,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
  },
  sectionTimingResetHint: {
    color: Colors.textSecondary,
    fontSize: 12,
    lineHeight: 17,
  },
  sectionTimingResetButton: {
    alignSelf: 'flex-start',
    paddingHorizontal: 12,
    paddingVertical: 8,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: Colors.brandPrimary,
    backgroundColor: Colors.surface,
  },
  sectionTimingResetButtonText: {
    color: Colors.brandPrimary,
    fontSize: 13,
    fontWeight: '700',
  },
  scrollContextBar: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
    marginBottom: 12,
    paddingHorizontal: 10,
    paddingVertical: 8,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
  },
  scrollContextLabel: {
    flex: 1,
    color: Colors.textPrimary,
    fontSize: 12,
    fontWeight: '800',
  },
  scrollContextMeta: { color: Colors.textMuted, fontSize: 11, fontWeight: '700' },
  swipeViewport: {
    overflow: 'hidden',
    position: 'relative',
  },
  swipePage: {
    width: '100%',
  },
  swipeOverlayPage: {
    position: 'absolute',
    top: 0,
    left: 0,
    right: 0,
    width: '100%',
  },
  waterBagRow: { flexDirection: 'row', gap: 8, flexWrap: 'wrap', marginBottom: 4 },
  waterBagBtn: {
    backgroundColor: Colors.surface,
    paddingHorizontal: 14,
    paddingVertical: 10,
    borderRadius: 999,
    borderWidth: 1,
    borderColor: Colors.border,
  },
  waterBagBtnActive: { backgroundColor: Colors.brandPrimary, borderColor: Colors.brandPrimary },
  waterBagBtnText: { color: Colors.textSecondary, fontSize: 14, fontWeight: '500' },
  waterBagBtnTextActive: { color: Colors.textOnBrand },
  waterBagHint: { color: Colors.textMuted, fontSize: 12, marginTop: -2, marginBottom: 8 },
  sectionActions: { flexDirection: 'row', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' },
  fillBtnContent: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  },
  fillBtn: {
    minHeight: 38,
    paddingHorizontal: 14,
    paddingVertical: 9,
    borderRadius: 999,
    borderWidth: 1,
    borderColor: Colors.brandPrimary,
    backgroundColor: Colors.brandPrimary,
    shadowColor: '#000000',
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.14,
    shadowRadius: 8,
    elevation: 3,
  },
  fillBtnLoading: {
    opacity: 0.96,
  },
  fillBtnText: { color: Colors.textOnBrand, fontSize: 13, fontWeight: '800' },
  fillBtnTextLoading: { maxWidth: 170, fontSize: 12 },
  fillBtnPremiumLocked: {
    borderColor: Colors.warning,
    backgroundColor: Colors.warningSurface,
    shadowOpacity: 0,
    elevation: 0,
  },
  fillBtnTextPremiumLocked: {
    color: Colors.warning,
  },
  addBtn: {
    backgroundColor: 'transparent',
    paddingHorizontal: 14,
    paddingVertical: 8,
    borderRadius: 8,
    borderWidth: 1.5,
    borderColor: Colors.brandLight,
  },
  addBtnText: { color: Colors.brandLight, fontSize: 14, fontWeight: '600' },
  emptyText: { color: Colors.textMuted, fontSize: 14, fontStyle: 'italic', textAlign: 'center', paddingVertical: 16 },
  stationCard: {
    backgroundColor: Colors.surface,
    borderRadius: 14,
    marginBottom: 12,
    borderWidth: 1,
    borderColor: Colors.border,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.07,
    shadowRadius: 6,
    elevation: 3,
  },
  stationCardFocused: {
    borderColor: Colors.brandPrimary,
    shadowOpacity: 0.12,
    shadowRadius: 10,
    elevation: 5,
  },
  stationCardCollapsedGreen: { backgroundColor: '#F1F8EC' },
  stationCardCollapsedOrange: { backgroundColor: '#FFF4E8' },
  stationCardCollapsedRed: { backgroundColor: '#FDECEC' },
  stationCardSystem: { backgroundColor: Colors.surfaceSecondary },
  stationHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  stationHeaderRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 12 },
  stationBadge: {
    minWidth: 34,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 999,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    paddingHorizontal: 8,
    paddingVertical: 4,
    marginRight: 8,
  },
  stationBadgeText: { color: Colors.textSecondary, fontSize: 11, fontWeight: '900' },
  stationBadgeMini: {
    minWidth: 30,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 999,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    paddingHorizontal: 7,
    paddingVertical: 3,
  },
  stationBadgeMiniText: { color: Colors.textSecondary, fontSize: 10, fontWeight: '900' },
  stationIcon: { fontSize: 16, marginRight: 8 },
  stationName: { flex: 1, color: Colors.textPrimary, fontWeight: '600', fontSize: 15 },
  stationPauseBadge: {
    borderRadius: 999,
    borderWidth: 1,
    borderColor: Colors.brandBorder,
    backgroundColor: Colors.brandSurface,
    paddingHorizontal: 8,
    paddingVertical: 4,
    marginRight: 8,
  },
  stationPauseText: { color: Colors.brandPrimary, fontSize: 11, fontWeight: '700' },
  stationKm: { color: Colors.textSecondary, fontSize: 13 },
  chevron: { color: Colors.textMuted, fontSize: 12, marginLeft: 8 },
  cardDivider: { height: 1, backgroundColor: Colors.border },
  coveragePanel: {
    marginHorizontal: 12,
    marginTop: 12,
    marginBottom: 6,
    paddingHorizontal: 12,
    paddingVertical: 12,
    borderRadius: 12,
    borderWidth: 1,
  },
  coveragePanelOk: {
    backgroundColor: '#F1F8EC',
    borderColor: '#CFE5BE',
  },
  coveragePanelWarning: {
    backgroundColor: '#FFF4E8',
    borderColor: '#F6D5AC',
  },
  coveragePanelDanger: {
    backgroundColor: '#FDECEC',
    borderColor: '#F4B9B9',
  },
  coveragePanelHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
    marginBottom: 8,
  },
  coveragePanelTitle: {
    flex: 1,
    color: Colors.textPrimary,
    fontSize: 14,
    fontWeight: '800',
  },
  coveragePanelDetail: {
    color: Colors.textPrimary,
    fontSize: 13,
    lineHeight: 18,
  },
  coveragePanelAction: {
    color: Colors.textSecondary,
    fontSize: 12,
    lineHeight: 17,
    marginTop: 6,
    fontWeight: '700',
  },
  coverageChipRow: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 6,
    marginTop: 10,
  },
  coverageChip: {
    borderRadius: 999,
    backgroundColor: 'rgba(255,255,255,0.78)',
    borderWidth: 1,
    borderColor: Colors.border,
    paddingHorizontal: 8,
    paddingVertical: 4,
  },
  coverageChipText: {
    color: Colors.textSecondary,
    fontSize: 11,
    fontWeight: '700',
  },
  coveragePill: {
    borderRadius: 999,
    paddingHorizontal: 9,
    paddingVertical: 4,
    borderWidth: 1,
  },
  coveragePillOk: {
    backgroundColor: '#E3F3D7',
    borderColor: '#B7D59C',
  },
  coveragePillWarning: {
    backgroundColor: '#FCE6C8',
    borderColor: '#EABF7B',
  },
  coveragePillDanger: {
    backgroundColor: '#F9D6D6',
    borderColor: '#E5A0A0',
  },
  coveragePillText: {
    fontSize: 11,
    fontWeight: '800',
    textTransform: 'uppercase',
    letterSpacing: 0.5,
  },
  coveragePillTextOk: {
    color: '#2D5016',
  },
  coveragePillTextWarning: {
    color: '#9A5B06',
  },
  coveragePillTextDanger: {
    color: '#B42318',
  },
  coverageCompactRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
    marginHorizontal: 10,
    marginTop: 10,
    marginBottom: 4,
    paddingHorizontal: 10,
    paddingVertical: 8,
    borderRadius: 10,
    borderWidth: 1,
  },
  coverageCompactTitle: {
    flex: 1,
    color: Colors.textPrimary,
    fontSize: 12,
    fontWeight: '700',
  },
  headerIconBtn: { padding: 6, borderRadius: 6, marginLeft: 2 },
  separator: { flexDirection: 'row', alignItems: 'center', marginVertical: 4, paddingHorizontal: 8 },
  separatorLine: { flex: 1, height: 1, backgroundColor: Colors.border },
  separatorText: { marginHorizontal: 10, color: Colors.textMuted, fontSize: 12 },
  separatorChevron: { color: Colors.textMuted, fontSize: 12, marginRight: 8 },
  sectionStationRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginTop: 4,
    marginBottom: 8,
    paddingHorizontal: 4,
    paddingVertical: 6,
    gap: 8,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: 'transparent',
  },
  sectionStationRowFocused: {
    backgroundColor: Colors.brandSurface,
    borderColor: Colors.brandBorder,
  },
  sectionStationLine: { flex: 1, height: 1, backgroundColor: Colors.border },
  sectionStationLabel: { color: Colors.textPrimary, fontSize: 13, fontWeight: '700', maxWidth: '50%' },
  sectionStationKm: { color: Colors.textMuted, fontSize: 12, fontWeight: '600' },
  sectionViewCard: {
    marginBottom: 14,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surface,
    paddingHorizontal: 12,
    paddingVertical: 10,
  },
  sectionViewHeader: { marginBottom: 8 },
  sectionViewTitle: { color: Colors.textPrimary, fontSize: 14, fontWeight: '700' },
  sectionViewMeta: { color: Colors.textSecondary, fontSize: 12, marginTop: 2 },
  profileCard: {
    marginBottom: 14,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surface,
    paddingHorizontal: 12,
    paddingVertical: 12,
  },
  profileSectionBlock: { marginBottom: 14 },
  profileSectionHeader: { paddingHorizontal: 4, marginBottom: 10 },
  profileHeader: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, marginBottom: 10 },
  profileHeaderText: { flex: 1 },
  profileTitle: { color: Colors.textPrimary, fontSize: 14, fontWeight: '700' },
  profileMeta: { color: Colors.textSecondary, fontSize: 12, marginTop: 2 },
  profileEmptyText: { color: Colors.textMuted, fontSize: 13, fontStyle: 'italic', marginBottom: 10 },
  profilePaceWrap: {
    width: 148,
    alignItems: 'stretch',
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.brandBorder,
    backgroundColor: Colors.brandSurface,
    paddingHorizontal: 10,
    paddingVertical: 8,
  },
  profilePaceLabel: { color: Colors.brandPrimary, fontSize: 11, fontWeight: '700', textAlign: 'center', marginBottom: 4 },
  profilePaceControlRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  profilePaceStepBtn: {
    width: 30,
    height: 30,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: Colors.brandBorder,
    backgroundColor: Colors.brandSurface,
    alignItems: 'center',
    justifyContent: 'center',
  },
  profilePaceStepBtnText: { color: Colors.brandPrimary, fontSize: 16, fontWeight: '700' },
  profilePaceInput: {
    flex: 1,
    minWidth: 62,
    borderRadius: 8,
    backgroundColor: Colors.surface,
    borderWidth: 1,
    borderColor: Colors.border,
    color: Colors.textPrimary,
    fontSize: 16,
    fontWeight: '700',
    textAlign: 'center',
    paddingHorizontal: 6,
    paddingVertical: 7,
  },
  profilePaceHint: { color: Colors.textMuted, fontSize: 10, marginTop: 4, textAlign: 'center' },
  profileChart: {
    height: 96,
    overflow: 'hidden',
    marginBottom: 10,
    borderRadius: 10,
    backgroundColor: Colors.surfaceSecondary,
    borderWidth: 1,
    borderColor: Colors.border,
  },
  profileChartEmpty: {
    height: 96,
    borderRadius: 10,
    backgroundColor: Colors.surfaceSecondary,
    borderWidth: 1,
    borderColor: Colors.border,
    marginBottom: 10,
  },
  profilePresetRow: { flexDirection: 'row', gap: 8, marginBottom: 12 },
  profilePresetBtn: {
    flex: 1,
    borderRadius: 999,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    paddingVertical: 8,
    alignItems: 'center',
  },
  profilePresetBtnText: { color: Colors.textPrimary, fontSize: 12, fontWeight: '700' },
  profileSegmentsList: { gap: 8 },
  profileSegmentRow: {
    flexDirection: 'row',
    gap: 10,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    paddingHorizontal: 10,
    paddingVertical: 10,
  },
  profileSegmentInfo: { flex: 1 },
  profileSegmentLabel: { color: Colors.textPrimary, fontSize: 13, fontWeight: '700' },
  profileSegmentMeta: { color: Colors.textSecondary, fontSize: 12, marginTop: 3 },
  profileSegmentTimeLabel: { color: Colors.textMuted, fontSize: 11, fontWeight: '600', marginTop: 8 },
  profileSegmentTime: { color: Colors.brandPrimary, fontSize: 24, fontWeight: '800', marginTop: 2, lineHeight: 28 },
  profileSegmentInputWrap: { width: 90, alignItems: 'stretch' },
  profileSegmentInputLabel: { color: Colors.textMuted, fontSize: 11, fontWeight: '600', marginBottom: 4, textAlign: 'center' },
  profileSegmentInput: {
    backgroundColor: Colors.surface,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: Colors.border,
    color: Colors.textPrimary,
    fontSize: 14,
    fontWeight: '600',
    textAlign: 'center',
    paddingHorizontal: 8,
    paddingVertical: 8,
  },
  profileSegmentInputHint: { color: Colors.textMuted, fontSize: 10, marginTop: 4, textAlign: 'center' },
  profileMetricsRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 },
  profileMetricPill: {
    borderRadius: 999,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    paddingHorizontal: 10,
    paddingVertical: 6,
  },
  profileMetricPillText: { color: Colors.textSecondary, fontSize: 12, fontWeight: '700' },
  profileSegmentControls: { marginTop: 2 },
  profileSegmentActions: { gap: 8 },
  profileActionBtn: {
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    paddingHorizontal: 12,
    paddingVertical: 10,
    alignItems: 'center',
    justifyContent: 'center',
  },
  profileActionBtnDisabled: { opacity: 0.45 },
  profileActionBtnText: { color: Colors.textPrimary, fontSize: 12, fontWeight: '700', textAlign: 'center' },
  profileDeleteBtn: { borderColor: '#F9A8A8', backgroundColor: '#FEF2F2' },
  profileDeleteBtnText: { color: '#DC2626', fontSize: 12, fontWeight: '700', textAlign: 'center' },
  sectionTimelineCard: {
    marginTop: 6,
    marginBottom: 10,
    marginHorizontal: 8,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surface,
    paddingVertical: 6,
    paddingHorizontal: 10,
  },
  sectionTimelineRow: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    gap: 10,
    paddingVertical: 6,
    borderBottomWidth: 1,
    borderBottomColor: Colors.border,
  },
  sectionTimelineRowLast: { borderBottomWidth: 0 },
  sectionTimelineTime: { width: 56, color: Colors.textSecondary, fontSize: 12, fontWeight: '700' },
  sectionTimelineInfo: { flex: 1 },
  sectionTimelineLabel: { color: Colors.textPrimary, fontSize: 13, fontWeight: '600' },
  sectionTimelineDetail: { color: Colors.textMuted, fontSize: 12, marginTop: 1 },
  sectionTimelineEmpty: { color: Colors.textMuted, fontSize: 13, fontStyle: 'italic', paddingVertical: 4 },
  collapsedGaugeRow: { paddingHorizontal: 10, paddingBottom: 10 },
  editModalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 20 },
  editModalBackdrop: { ...StyleSheet.absoluteFillObject },
  editModalCard: { backgroundColor: Colors.surface, borderRadius: 16, width: '100%', maxHeight: '92%', overflow: 'hidden' },
  editModalContent: { padding: 24 },
  editModalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 },
  editModalTitle: { fontSize: 17, fontWeight: '700', color: Colors.textPrimary },
  editModalCloseButton: { width: 44, height: 44, borderRadius: 22 },
  stationServiceOptions: {
    flexDirection: 'row',
    gap: 8,
  },
  stationServiceOption: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
    gap: 5,
    minHeight: 76,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    paddingHorizontal: 6,
    paddingVertical: 8,
  },
  stationServiceOptionActive: {
    borderColor: Colors.brandBorder,
    backgroundColor: Colors.brandSurface,
  },
  stationServiceLabel: {
    color: Colors.textSecondary,
    fontSize: 12,
    fontWeight: '800',
    textAlign: 'center',
  },
  stationServiceLabelActive: {
    color: Colors.brandPrimary,
  },
  stationServiceChipRow: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 6,
    paddingHorizontal: 14,
    paddingBottom: 10,
    marginTop: -2,
  },
  stationServiceChip: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
    borderRadius: 999,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    paddingHorizontal: 8,
    paddingVertical: 4,
  },
  stationServiceChipActive: {
    borderColor: Colors.brandBorder,
    backgroundColor: Colors.brandSurface,
  },
  stationServiceChipText: {
    color: Colors.textSecondary,
    fontSize: 11,
    fontWeight: '800',
  },
  stationServiceChipTextActive: {
    color: Colors.brandPrimary,
  },
  suppliesDisabledBox: {
    borderTopWidth: 1,
    borderTopColor: Colors.border,
    paddingHorizontal: 14,
    paddingVertical: 12,
  },
  suppliesDisabledText: {
    color: Colors.textSecondary,
    fontSize: 13,
    lineHeight: 18,
    fontWeight: '600',
  },
  stationOrderButtons: { flexDirection: 'row', gap: 4, marginRight: 8 },
  orderBtn: {
    backgroundColor: Colors.surfaceSecondary,
    borderWidth: 1,
    borderColor: Colors.border,
    width: 28,
    height: 28,
    borderRadius: 6,
    justifyContent: 'center',
    alignItems: 'center',
  },
  orderBtnDisabled: { opacity: 0.3 },
  orderBtnText: { color: Colors.textSecondary, fontSize: 14 },
  stationIndex: { flex: 1, color: Colors.textSecondary, fontSize: 13, fontWeight: '600' },
  stationSystemLabel: { flex: 1, color: Colors.brandPrimary, fontSize: 14, fontWeight: '700' },
  stationLockedKm: { color: Colors.textSecondary, fontSize: 14, fontWeight: '500', marginBottom: 4 },
  arriveeNote: { color: Colors.textMuted, fontSize: 13, fontStyle: 'italic', marginTop: 4 },
  removeBtn: { width: 28, height: 28, borderRadius: 14, backgroundColor: Colors.dangerSurface, justifyContent: 'center', alignItems: 'center' },
  removeBtnText: { color: Colors.danger, fontSize: 13, fontWeight: '700' },
  gaugesRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'flex-start',
    paddingVertical: 14,
    paddingHorizontal: 8,
    gap: 8,
  },
  gaugesRowCompact: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    paddingVertical: 2,
    paddingHorizontal: 0,
    gap: 6,
  },
  gaugeCard: { flex: 1, alignItems: 'center' },
  gaugeLabel: { fontSize: 10, color: Colors.textSecondary, marginBottom: 8, textAlign: 'center', fontWeight: '700' },
  gaugeValueCaption: { fontSize: 9, color: Colors.textMuted, marginTop: 6, textAlign: 'center', fontWeight: '700' },
  gaugeValue: { fontSize: 10, color: Colors.textPrimary, marginTop: 6, textAlign: 'center', fontWeight: '700' },
  suppliesSection: { borderTopWidth: 1, borderTopColor: Colors.border, paddingTop: 10, paddingHorizontal: 14, paddingBottom: 6 },
  suppliesHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  suppliesLabel: { color: Colors.textSecondary, fontSize: 13, fontWeight: '600' },
  suppliesAddBtn: { color: Colors.brandPrimary, fontSize: 13, fontWeight: '600' },
  suppliesEmpty: { color: Colors.textMuted, fontSize: 13, fontStyle: 'italic' },
  supplyRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 6, gap: 8 },
  supplyInfo: { flex: 1 },
  supplyName: { color: Colors.textPrimary, fontSize: 14, fontWeight: '600' },
  supplyType: { color: Colors.textMuted, fontSize: 11, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5 },
  supplyMeta: { color: Colors.textSecondary, fontSize: 12, marginTop: 2 },
  supplyControls: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  qtyBtn: {
    backgroundColor: Colors.surface,
    borderWidth: 1,
    borderColor: Colors.border,
    width: 28,
    height: 28,
    borderRadius: 6,
    justifyContent: 'center',
    alignItems: 'center',
  },
  qtyBtnDisabled: { opacity: 0.3 },
  qtyBtnText: { color: Colors.textPrimary, fontSize: 16, fontWeight: '600' },
  qtyText: { color: Colors.textPrimary, fontWeight: '700', minWidth: 24, textAlign: 'center', fontSize: 15 },
  removeSupplyBtn: { paddingLeft: 8 },
  removeSupplyText: { color: Colors.danger, fontSize: 18 },
  saveButton: { backgroundColor: Colors.brandPrimary, borderRadius: 12, paddingVertical: 18, alignItems: 'center', marginTop: 32 },
  saveButtonDisabled: { opacity: 0.6 },
  saveButtonText: { color: Colors.textOnBrand, fontSize: 17, fontWeight: '700' },
  saveSpacer: { height: 88 },
  floatingSaveButtonAnchor: {
    position: 'absolute',
    right: 20,
    bottom: 24,
    width: 56,
    height: 56,
  },
  floatingSaveButton: {
    backgroundColor: Colors.brandPrimary,
    borderRadius: 999,
    width: 56,
    height: 56,
    alignItems: 'center',
    justifyContent: 'center',
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.2,
    shadowRadius: 8,
    elevation: 8,
  },
  modalWrapper: { flex: 1, justifyContent: 'flex-end' },
  modalOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.5)' },
  settingsSheet: {
    backgroundColor: Colors.surface,
    borderTopLeftRadius: 24,
    borderTopRightRadius: 24,
    paddingHorizontal: 20,
    paddingTop: 8,
    paddingBottom: 24,
    maxHeight: '88%',
  },
  settingsSheetHandle: {
    alignSelf: 'center',
    width: 42,
    height: 4,
    borderRadius: 999,
    backgroundColor: Colors.borderStrong,
    marginBottom: 14,
  },
  settingsSheetHeader: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    gap: 12,
    marginBottom: 14,
  },
  settingsSheetHeaderCopy: {
    flex: 1,
  },
  settingsSheetTitle: {
    color: Colors.textPrimary,
    fontSize: 18,
    fontWeight: '800',
    lineHeight: 22,
  },
  settingsSheetSubtitle: {
    color: Colors.textSecondary,
    fontSize: 12,
    fontWeight: '600',
    marginTop: 4,
  },
  settingsSheetContent: {
    paddingBottom: 18,
  },
  settingsSheetDoneButton: {
    alignItems: 'center',
    justifyContent: 'center',
    minHeight: 46,
    borderRadius: 12,
    backgroundColor: Colors.brandPrimary,
    marginTop: 18,
  },
  settingsSheetDoneButtonText: {
    color: Colors.textOnBrand,
    fontSize: 15,
    fontWeight: '800',
  },
  pickerSheet: { backgroundColor: Colors.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, paddingBottom: 40, maxHeight: '80%' },
  pickerHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  pickerTitle: { fontSize: 17, fontWeight: '700', color: Colors.textPrimary },
  pickerCloseBtn: { width: 32, height: 32, borderRadius: 16, backgroundColor: Colors.surfaceSecondary, justifyContent: 'center', alignItems: 'center' },
  pickerCloseText: { color: Colors.textSecondary, fontSize: 14, fontWeight: '700' },
  autoFillLimitsSheet: {
    backgroundColor: Colors.surface,
    borderTopLeftRadius: 24,
    borderTopRightRadius: 24,
    paddingHorizontal: 20,
    paddingTop: 8,
    paddingBottom: 28,
    maxHeight: '84%',
  },
  autoFillLimitsHeader: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    gap: 12,
    marginBottom: 14,
  },
  autoFillLimitsHeaderCopy: { flex: 1 },
  autoFillLimitsTitle: {
    color: Colors.textPrimary,
    fontSize: 18,
    fontWeight: '800',
    lineHeight: 22,
  },
  autoFillLimitsSubtitle: {
    color: Colors.textSecondary,
    fontSize: 12,
    fontWeight: '600',
    lineHeight: 17,
    marginTop: 4,
  },
  autoFillLimitsLoading: {
    marginVertical: 28,
  },
  autoFillLimitsList: {
    paddingBottom: 8,
  },
  autoFillLimitRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
    minHeight: 62,
    borderBottomWidth: 1,
    borderBottomColor: Colors.border,
    paddingVertical: 9,
  },
  autoFillLimitCheckbox: {
    width: 34,
    height: 34,
    alignItems: 'center',
    justifyContent: 'center',
  },
  autoFillLimitInfo: {
    flex: 1,
    minWidth: 0,
  },
  autoFillLimitNameRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  },
  autoFillLimitName: {
    flex: 1,
    color: Colors.textPrimary,
    fontSize: 14,
    fontWeight: '700',
  },
  autoFillLimitBadge: {
    color: Colors.textMuted,
    fontSize: 10,
    fontWeight: '800',
    textTransform: 'uppercase',
  },
  autoFillLimitBadgeActive: {
    color: Colors.brandPrimary,
  },
  autoFillLimitMeta: {
    color: Colors.textSecondary,
    fontSize: 12,
    fontWeight: '600',
    marginTop: 3,
  },
  autoFillLimitInput: {
    width: 78,
    minHeight: 42,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.brandBorder,
    backgroundColor: Colors.surfaceSecondary,
    color: Colors.textPrimary,
    fontSize: 15,
    fontWeight: '700',
    paddingHorizontal: 10,
    textAlign: 'center',
  },
  autoFillLimitInputDisabled: {
    borderColor: Colors.border,
    color: Colors.textMuted,
  },
  autoFillLimitsActions: {
    flexDirection: 'row',
    gap: 10,
    marginTop: 16,
  },
  autoFillLimitsSecondaryButton: {
    flex: 1,
    minHeight: 46,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 12,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
  },
  autoFillLimitsPrimaryButton: {
    flex: 1.35,
    minHeight: 46,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 12,
    backgroundColor: Colors.brandPrimary,
  },
  autoFillLimitsPrimaryButtonDisabled: {
    opacity: 0.55,
  },
  autoFillLimitsSecondaryText: {
    color: Colors.textSecondary,
    fontSize: 14,
    fontWeight: '800',
  },
  autoFillLimitsPrimaryText: {
    color: Colors.textOnBrand,
    fontSize: 14,
    fontWeight: '800',
  },
  autoFillPreview: {
    gap: 12,
    paddingVertical: 8,
  },
  autoFillPreviewMetrics: {
    flexDirection: 'row',
    gap: 8,
  },
  autoFillPreviewMetric: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
    minHeight: 76,
    borderRadius: 14,
    backgroundColor: Colors.surfaceSecondary,
  },
  autoFillPreviewValue: {
    color: Colors.textPrimary,
    fontSize: 22,
    fontWeight: '900',
  },
  autoFillPreviewLabel: {
    color: Colors.textSecondary,
    fontSize: 11,
    fontWeight: '700',
    marginTop: 2,
  },
  autoFillPreviewChange: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
    minHeight: 42,
    paddingHorizontal: 12,
    borderRadius: 12,
    backgroundColor: Colors.brandSurface,
  },
  autoFillPreviewChangeText: {
    color: Colors.brandPrimary,
    fontSize: 13,
    fontWeight: '800',
  },
  autoFillPreviewWarning: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
    minHeight: 52,
    paddingHorizontal: 12,
    borderRadius: 12,
    backgroundColor: Colors.warningSurface,
  },
  autoFillPreviewWarningCopy: { flex: 1 },
  autoFillPreviewWarningTitle: {
    color: Colors.textPrimary,
    fontSize: 12,
    fontWeight: '800',
  },
  autoFillPreviewWarningText: {
    color: Colors.warning,
    fontSize: 12,
    fontWeight: '800',
    marginTop: 2,
  },
  autoFillPreviewOk: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
    minHeight: 42,
    paddingHorizontal: 12,
    borderRadius: 12,
    backgroundColor: Colors.brandSurface,
  },
  autoFillPreviewOkText: {
    color: Colors.success,
    fontSize: 13,
    fontWeight: '800',
  },
  autoFillUndoBanner: {
    position: 'absolute',
    right: 14,
    bottom: 76,
    left: 14,
    zIndex: 19,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    minHeight: 44,
    paddingHorizontal: 14,
    borderRadius: 12,
    backgroundColor: Colors.textPrimary,
    boxShadow: '0 6px 16px rgba(26, 26, 26, 0.2)',
  },
  autoFillUndoText: {
    color: Colors.textOnBrand,
    fontSize: 13,
    fontWeight: '700',
  },
  autoFillUndoAction: {
    color: Colors.brandLight,
    fontSize: 13,
    fontWeight: '900',
  },
  pickerSearchInput: {
    backgroundColor: Colors.surfaceSecondary,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.border,
    paddingHorizontal: 12,
    paddingVertical: 10,
    fontSize: 15,
    color: Colors.textPrimary,
    marginBottom: 12,
  },
  pickerSortRow: {
    flexDirection: 'row',
    gap: 8,
    marginBottom: 8,
  },
  pickerSortBtn: {
    flex: 1,
    backgroundColor: Colors.surfaceSecondary,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: Colors.border,
    paddingVertical: 9,
    alignItems: 'center',
  },
  pickerSortBtnActive: {
    backgroundColor: Colors.brandSurface,
    borderColor: Colors.brandBorder,
  },
  pickerSortBtnText: {
    color: Colors.textSecondary,
    fontSize: 12,
    fontWeight: '700',
  },
  pickerSortBtnTextActive: {
    color: Colors.brandPrimary,
  },
  pickerSectionTitle: {
    color: Colors.textMuted,
    fontSize: 11,
    fontWeight: '700',
    textTransform: 'uppercase',
    letterSpacing: 0.8,
    marginTop: 12,
    marginBottom: 6,
  },
  pickerEmpty: { color: Colors.textMuted, fontSize: 14, fontStyle: 'italic', textAlign: 'center', paddingVertical: 12 },
  pickerRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: Colors.border },
  pickerProductImageFrame: {
    position: 'relative',
    width: 46,
    height: 46,
    marginRight: 12,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: Colors.border,
    backgroundColor: Colors.surfaceSecondary,
    overflow: 'visible',
  },
  pickerProductImage: {
    width: '100%',
    height: '100%',
    borderRadius: 11,
    resizeMode: 'contain',
  },
  pickerProductImagePlaceholder: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
  },
  pickerVerifiedBadge: {
    position: 'absolute',
    top: -5,
    right: -5,
    width: 22,
    height: 22,
    borderRadius: 11,
    alignItems: 'center',
    justifyContent: 'center',
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.08,
    shadowRadius: 2,
    elevation: 2,
  },
  pickerVerifiedBadgeImage: {
    width: '100%',
    height: '100%',
    resizeMode: 'contain',
  },
  pickerRowInfo: { flex: 1, marginRight: 12 },
  pickerRowName: { color: Colors.textPrimary, fontSize: 14, fontWeight: '600' },
  pickerRowType: { color: Colors.textMuted, fontSize: 11, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 1 },
  pickerRowMeta: { color: Colors.textSecondary, fontSize: 12, marginTop: 3, fontWeight: '600' },
  pickerAddBtn: { backgroundColor: 'transparent', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, borderWidth: 1.5, borderColor: Colors.brandPrimary },
  pickerAddBtnDone: { borderColor: Colors.brandPrimary, backgroundColor: Colors.brandPrimary },
  pickerAddBtnText: { color: Colors.brandPrimary, fontSize: 12, fontWeight: '600' },
  pickerAddBtnTextDone: { color: Colors.textOnBrand },
});
