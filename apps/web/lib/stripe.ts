...
const stripeConfigSchema = z.object({
  ...
  organizerEssentialPriceId: z.string().trim().min(1).optional(),
  organizerCompletePriceId: z.string().trim().min(1).optional(),
  organizerSignaturePriceId: z.string().trim().min(1).optional(),
  organizerEssentialToCompletePriceId: z.string().trim().min(1).optional(),
  organizerEssentialToSignaturePriceId: z.string().trim().min(1).optional(),
  organizerCompleteToSignaturePriceId: z.string().trim().min(1).optional(),
...
export const getStripeConfig = (): StripeConfig | null => {
  const parsed = stripeConfigSchema.safeParse({
    ...
    organizerEssentialPriceId: process.env.STRIPE_ORGANIZER_ESSENTIAL_PRICE_ID,
    organizerCompletePriceId: process.env.STRIPE_ORGANIZER_COMPLETE_PRICE_ID,
    organizerSignaturePriceId: process.env.STRIPE_ORGANIZER_SIGNATURE_PRICE_ID,
    organizerEssentialToCompletePriceId: process.env.STRIPE_ORGANIZER_ESSENTIAL_TO_COMPLETE_PRICE_ID,
    organizerEssentialToSignaturePriceId: process.env.STRIPE_ORGANIZER_ESSENTIAL_TO_SIGNATURE_PRICE_ID,
    organizerCompleteToSignaturePriceId: process.env.STRIPE_ORGANIZER_COMPLETE_TO_SIGNATURE_PRICE_ID,
