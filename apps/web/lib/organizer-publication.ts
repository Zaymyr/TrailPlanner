...
export async function validateOrganizerEventPublication(
  ...
  if (!currentEdition?.start_date) return { ok: false, message: "Ajoute une date de debut avant de demander la publication.", status: 409 };
  if (!currentEdition.end_date) return { ok: false, message: "Ajoute une date de fin avant de demander la publication.", status: 409 };
