...
} from "./social-instagram-template-draft";
import { socialRacePlanTemplateSchema } from "./social-race-plan-template";

const baseTemplate = socialRacePlanTemplateSchema.parse({
  schemaVersion: 1,
...
describe("social Instagram template draft", () => {
  it("maps the canonical payload into editable Instagram draft fields", () => {
    const draft = buildSocialInstagramTemplateDraft(baseTemplate);
    ...
    expect(draft.avgWaterMl).toBe("500");
    expect(draft.aidStations[0]).toMatchObject({
      name: "Aid 1",
      km: "24.5",
      eta: "4h10",
    });
  ...
  it("builds and reapplies local overrides without losing untouched defaults", () => {
    const defaults = buildSocialInstagramTemplateDraft(baseTemplate);
    const edited = {
      ...defaults,
      tagline: "Mon ravito ne s'improvise pas.",
      accentKey: "earth" as const,
      aidStations: [
        ...defaults.aidStations,
        { name: "Finish", km: "85.9", eta: "14h44", take: "Recup" },
      ],
    };
