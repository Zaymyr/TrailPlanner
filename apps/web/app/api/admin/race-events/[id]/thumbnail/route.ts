...
export async function PUT(request: NextRequest, context: { params: { id?: string } }) {
  ...
  const rateLimit = checkRateLimit(`race-event-thumbnail-update:${supabaseUser.id}`, 10, 60_000);
  ...
  if (!uploadResponse.ok) {
    console.error("Unable to upload race event thumbnail", await uploadResponse.text());
  ...
  if (!updateResponse.ok) {
    console.error("Unable to update race event thumbnail_url", await updateResponse.text());
    ...
    return withSecurityHeaders(NextResponse.json({ message: "Unable to update race event thumbnail." }, { status: 502 }));
  }
