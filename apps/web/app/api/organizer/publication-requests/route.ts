...
export async function POST(request: NextRequest) {
  ...
  const readiness = await validateOrganizerEventPublication(auth.serviceConfig, parsed.data.eventId);
  if (!readiness.ok) return jsonError(readiness.message, readiness.status);

  const existingResponse = await fetch(
    `${auth.serviceConfig.supabaseUrl}/rest/v1/race_event_publication_requests?event_id=eq.${parsed.data.eventId}&status=eq.pending&select=id&limit=1`,
  ...
  if (((await existingResponse.json()) as unknown[]).length > 0) {
    return jsonError("Une demande de publication est deja en attente.", 409);
  }
  ...
  const insertResponse = await fetch(`${auth.serviceConfig.supabaseUrl}/rest/v1/race_event_publication_requests`, {
    ...
    body: JSON.stringify({ user_id: auth.user.id, event_id: parsed.data.eventId, status: "pending" }),
    cache: "no-store",
  });
