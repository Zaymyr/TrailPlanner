...
export function reconcileRemoteListSave<T>({
  ...
  const hasNewerChanges = currentRevision > savedRevision;
  return {
    items: hasNewerChanges ? currentItems : savedItems,
    dirty: hasNewerChanges,
    saveAgain: hasNewerChanges,
  };
}
...
function useRemoteList<T>(url: string | null, key: string, headers: Headers, initial: T[] = []) {
  ...
  const saveLatest = useCallback(async () => {
    ...
    try {
      ...
      savingRef.current = false;
      if (queuedRef.current || revisionRef.current > savedRevision) {
        queuedRef.current = false;
  ...
  useEffect(() => {
    if (!url || !loaded || !dirty) return;
    const timer = window.setTimeout(() => void saveLatest(), 700);
    return () => window.clearTimeout(timer);
  }, [dirty, items, loaded, saveLatest, url]);
...
const Select = ({ value, onChange, children, label }: { value:string; onChange:(value:string)=>void; children:ReactNode; label:string }) => (
  <select aria-label={label} className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm" value={value} onChange={(event)=>onChange(event.target.value)}>{children}</select>
);
