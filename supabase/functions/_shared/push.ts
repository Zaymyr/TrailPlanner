...
export async function authenticateRequestUser(request: Request) {
  ...
  if (error || !data.user?.id) {
    console.error("Unable to authenticate push registration request.", error);
    return null;
  }
...
async function fetchExistingDedupes(pushDeviceIds: string[], dedupeKeys: string[]) {
  ...
  return new Set(rows.map((row) => `${row.push_device_id}:${row.dedupe_key}`));
}

async function storeNotificationEvents(reminders: PendingReminder[], results: ExpoPushResult[]) {
  ...
  const { error } = await serviceClient.from("push_notification_events").upsert(rows, {
